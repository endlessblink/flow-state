...
### 5. Individual Document Storage Pattern (CRITICAL)

**Problem**: Storing all entities in a single document (e.g., `tasks:data` array) causes sync conflicts when different browsers edit different items simultaneously.

**Solution**: Store each entity as a separate document with ID prefix (e.g., `task-{id}`).
...
export const loadAllTasks = async (db: PouchDB.Database): Promise<Task[]> => {
  const result = await db.allDocs({
    include_docs: true,
    startkey: TASK_DOC_PREFIX,
    endkey: `${TASK_DOC_PREFIX}\ufff0`  // Range query for all task-* docs
  })

  return result.rows
    .filter(row => row.doc && 'data' in row.doc)
    .map(row => (row.doc as any).data)
}

export const deleteTask = async (db: PouchDB.Database, taskId: string) => {
...
| Single Array Document | Individual Documents |
|-----------------------|---------------------|
| ❌ Edit Task A in Browser 1, Task B in Browser 2 = **CONFLICT** | ✅ No conflict - different documents |
| ❌ Sync entire array on any change | ✅ Sync only changed entity |
| ❌ Last-write-wins loses data | ✅ Parallel edits work safely |
| ❌ Large replication payloads | ✅ Minimal bandwidth per change |

#### Migration from Legacy Format

```typescript
export const migrateFromLegacyFormat = async (db: PouchDB.Database) => {
  try {
    const legacyDoc = await db.get('tasks:data') as any
    if (legacyDoc?.data && Array.isArray(legacyDoc.data)) {
      // Save each task as individual document
      await saveTasks(db, legacyDoc.data)
