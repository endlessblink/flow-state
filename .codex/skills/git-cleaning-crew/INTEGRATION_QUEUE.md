...
- `already-present`: `fix/runtime-contract-tests` — current `main` retains the stronger updater retry and process-replacement contracts and the corrected day-group membership assertion; the archived test-only ref would remove coverage and weaken the replacement assertion.
- `archived`: `codex/h1-session-ack` — the session-ack protocol is absent from current `main`, but the archived change spans 11 Electron/sidecar/auth files and requires authenticated E2E proof; it remains preserved for a separately authorized runtime integration.
