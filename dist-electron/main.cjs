  ...
  "node_modules/electron-updater/node_modules/fs-extra/lib/ensure/file.js"(exports2, module2) {
    ...
    function createFileSync(file) {
      ...
      try {
        ...
        if (err && err.code === "ENOENT") mkdir3.mkdirsSync(dir);
        else throw err;
      }
...
// node_modules/builder-util-runtime/node_modules/sax/lib/sax.js
var require_sax = __commonJS({
  "node_modules/builder-util-runtime/node_modules/sax/lib/sax.js"(exports2) {
    (function(sax) {
...
function registerShellHandlers() {
  import_electron.ipcMain.handle("shell:openExternal", async (_event, url) => {
...
function registerDialogHandlers() {
  import_electron2.ipcMain.handle("dialog:showSave", async (_event, options) => {
    const win = import_electron2.BrowserWindow.getFocusedWindow();
    if (!win) return { canceled: true, filePath: void 0 };
    return import_electron2.dialog.showSaveDialog(win, options);
  });
  import_electron2.ipcMain.handle("dialog:showOpen", async (_event, options) => {
    const win = import_electron2.BrowserWindow.getFocusedWindow();
    if (!win) return { canceled: true, filePaths: [] };
    return import_electron2.dialog.showOpenDialog(win, options);
  });
}
...
function registerFsHandlers() {
  import_electron3.ipcMain.handle("fs:readFile", async (_event, path) => {
    return (0, import_promises.readFile)(path, "utf-8");
  });
  import_electron3.ipcMain.handle("fs:writeFile", async (_event, path, data) => {
    await (0, import_promises.writeFile)(path, data, "utf-8");
  });
  import_electron3.ipcMain.handle("fs:exists", async (_event, path) => {
    try {
  ...
  import_electron3.ipcMain.handle("fs:mkdir", async (_event, path) => {
...
var storePath = () => (0, import_path.join)(import_electron4.app.getPath("userData"), "store.json");
var storeData = {};
var loaded = false;
...
async function saveStore() {
  ...
  await (0, import_promises2.writeFile)(path, JSON.stringify(storeData, null, 2), "utf-8");
}
function registerStoreHandlers() {
  import_electron4.ipcMain.handle("store:get", async (_event, key) => {
    await loadStore();
    return storeData[key] ?? null;
  });
  import_electron4.ipcMain.handle("store:set", async (_event, key, value) => {
    await loadStore();
    storeData[key] = value;
    await saveStore();
  });
...
// electron/ipc/http.ts
var import_electron5 = require("electron");
function registerHttpHandlers() {
  import_electron5.ipcMain.handle("http:fetch", async (_event, url, options) => {
    const resp = await import_electron5.net.fetch(url, {
...
function registerWindowHandlers() {
  ...
  import_electron6.ipcMain.handle("window:maximize", () => {
    const win = import_electron6.BrowserWindow.getFocusedWindow();
    if (win?.isMaximized()) {
      win.unmaximize();
    } else {
      win?.maximize();
  ...
  import_electron6.ipcMain.handle("window:close", () => {
    import_electron6.BrowserWindow.getFocusedWindow()?.close();
  });
}
...
function registerUpdater() {
  ...
  import_electron_updater.autoUpdater.on("update-available", (info) => {
    const win = import_electron7.BrowserWindow.getAllWindows()[0];
  ...
  import_electron7.ipcMain.handle("updater:check", async () => {
    try {
  ...
  import_electron7.ipcMain.handle("updater:download", async () => {
    await import_electron_updater.autoUpdater.downloadUpdate();
  });
  import_electron7.ipcMain.handle("updater:install", () => {
...
var OAUTH_PORTS = [24895, 24896, 24897];
...
function registerOAuthHandlers() {
  import_electron8.ipcMain.handle("oauth:start", async () => {
    if (activeServer) {
      activeServer.close();
      activeServer = null;
    }
  ...
  import_electron8.ipcMain.handle("oauth:waitForCallback", async () => {
    if (!activeServer) {
  ...
  import_electron8.ipcMain.handle("oauth:cancel", async () => {
    if (activeServer) {
      activeServer.close();
      activeServer = null;
    }
...
// electron/main.ts
import_electron9.app.setName("flow-state");
var gotLock = import_electron9.app.requestSingleInstanceLock();
if (!gotLock) {
...
function createWindow() {
  mainWindow = new import_electron9.BrowserWindow({
    ...
    minHeight: 600,
    title: "FlowState",
    icon: (0, import_path2.join)(__dirname, "../src-tauri/icons/icon.png"),
    webPreferences: {
      preload: (0, import_path2.join)(__dirname, "preload.cjs"),
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    const indexPath = (0, import_path2.join)(__dirname, "../dist/index.html");
...
import_electron9.ipcMain.handle("app:getVersion", () => import_electron9.app.getVersion());
