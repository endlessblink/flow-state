...
- Start here in the next AI lane session before moving to TASK-1856. The task/subtask preview/apply/rollback core exists, but the audit trail is still local-first and the command model only covers task/subtask creation.
- Next target: back the audit trail with the existing Supabase/IndexedDB persistence patterns so applied command batches survive reload/sync and can be queried from the future command center.
- Then extend typed commands and semantic duplicate decisions beyond tasks/subtasks to lanes/groups, calendar/focus blocks, canvas layout proposals, and memory/feedback events.
- After persistence and broader command kinds, wire real AI cards/tool writes through the command substrate instead of direct apply buttons.
