...
- Start here in the next AI lane session before moving to TASK-1856. The task/subtask update/create/delete, lane create, calendar schedule, canvas group-create/canvas node-move, memory patch, and recommendation-feedback preview/apply/rollback core exists; audit/rollback records survive reload through IndexedDB; real `ChatMessage` recommendation feedback, weekly-question/clarification memory patch, and smart-lane apply consumers route through the shared substrate; and `src/services/ai/tools.ts` no longer has direct task/canvas store writes for AI tool mutations.
- Next target: decide and implement policy for the two remaining `ChatMessage` undo-backed AI card paths: weekly follow-up explicit duplicate override (`createTaskWithUndo`) and day-plan apply (`bulkUpdateTasksWithUndo`). Either document them as explicit user-confirmed normal undo flows outside TASK-1855's AI-proposal substrate requirement, or convert them to command-substrate consumers too.
- Do not begin TASK-1856 command-center UI until the direct-write audit is complete enough to define the command-center surface around real converted consumers rather than a parallel UI abstraction.
