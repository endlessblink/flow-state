...
**Follow-up release proof**: Electron 1.4.271 supersedes 1.4.270 after the installed-profile reconnect repro. It passed the full ship gate, Electron sync guard, type checks, package validation, exact recovery regressions, and two independent release-blocker reviews. The production updater manifest serves 1.4.271; both AppImage and deb respond with HTTP 206 and byte sizes matching the manifest.
