...
**Shipped** — a 4-layer blank-screen/package-startup gate, each proven to fail on a real break and pass on a good build:
...
4. **Packaged AppImage startup smoke (runtime)** — `scripts/smoke-electron-appimage.sh` launches the actual built AppImage with logging and fails the ship path on main-process module/load crashes (`Cannot find module`, `MODULE_NOT_FOUND`, etc.) or any early startup exit. This catches the packaged `electron-updater -> fs-extra -> universalify` dependency crash that renderer smoke cannot see.

**Wired as BLOCKING gates into every ship path**: `scripts/deploy-electron-update.sh` (Step 0 type-check + Step 1b render smoke + file:// check + post-package AppImage smoke, aborts before upload), the `electron:build` npm script, CI `deploy.yml` (VPS web deploy), and CI `ci.yml` (PR time — earliest renderer gate). Runners: `scripts/verify-build-renders.sh` (`npm run verify:build`) and `scripts/smoke-electron-appimage.sh` (`npm run electron:smoke-appimage`). SOP: `docs/sop/SOP-070-blank-screen-gate.md`.
