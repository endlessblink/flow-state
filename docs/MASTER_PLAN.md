...
- Start here in the next AI lane session before moving to TASK-1856. The task/subtask update/create, lane create, calendar schedule, canvas group-create/canvas node-move, memory patch, and recommendation-feedback preview/apply/rollback core exists; audit/rollback records survive reload through IndexedDB; and real `ChatMessage` recommendation feedback, weekly-question/clarification memory patch, and smart-lane apply consumers now route through the shared substrate with regression/browser proof.
- Next target: convert the remaining direct AI write surfaces in `src/services/ai/tools.ts` task/lane/canvas mutations. Separately audit whether the remaining `ChatMessage` day-plan apply and weekly follow-up duplicate-override undo paths should stay as explicit user-confirmed normal undo flows or become command-substrate consumers too.
