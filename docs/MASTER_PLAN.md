...
- Start here in the next AI lane session before moving to TASK-1856. The task/subtask/lane/calendar schedule/canvas group-create preview/apply/rollback core exists and audit/rollback records survive reload through IndexedDB, but canvas layout/move proposals and memory/feedback command families are still missing.
- Next target: extend typed commands and semantic duplicate decisions to canvas layout/move proposals and memory/feedback events.
- After those command kinds, wire real AI cards/tool writes through the command substrate instead of direct apply buttons.
