...
- Start here in the next AI lane session before moving to TASK-1856. The task/subtask/lane/calendar schedule/canvas group-create/canvas node-move/memory patch/recommendation feedback preview/apply/rollback core exists, audit/rollback records survive reload through IndexedDB, and real `ChatMessage` recommendation feedback plus weekly-question/clarification memory patch consumers now route through the shared substrate with localhost/browser proof.
- Next target: convert the remaining direct AI write surfaces, especially `ChatMessage` smart-lane writes plus `src/services/ai/tools.ts` task/lane/canvas mutations, so no one-off AI apply/write button bypasses preview/apply/audit.
