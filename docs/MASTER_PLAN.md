...
- Start here in the next AI lane session before moving to TASK-1856. The task/subtask preview/apply/rollback core exists and audit/rollback records now survive reload through IndexedDB, but the command model still only covers task/subtask creation.
- Next target: extend typed commands and semantic duplicate decisions beyond tasks/subtasks to lanes/groups, calendar/focus blocks, canvas layout proposals, and memory/feedback events.
- After broader command kinds, wire real AI cards/tool writes through the command substrate instead of direct apply buttons.
