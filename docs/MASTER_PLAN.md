...
**2026-07-16 deployment progress**: Electron 1.4.267 and the canonical database migrations are deployed. The live package advertises all 16 required route families; the bundled-sidecar validator, reliable-assistant database gate, 3,761-test unit suite, package validation, production function/privilege checks, and independent release review pass. After desktop reauthentication, the installed Hermes connector passed 19 signed-user read/preview checks plus real deleted-task restore, status-change, recurring completion, and all three existing-work-block action dispatches without an apply. The audit exposed a recurrence-heavy overlap scan that could hit the production statement timeout and a connector validator that rejected legacy recurrence occurrences; a forward optimization migration now materializes timezone names once per command, the mixed-instance validator is deployed, an 800-occurrence five-second regression passes, and two formerly failing recurring-task previews complete in 0.26–0.27 seconds. Final production apply/replay/read-back/delete proof remains approval-gated, so this task stays open rather than claiming mutation completion from previews alone.
