...
- Start here in the next AI lane session before moving to TASK-1856. The task/subtask/lane/calendar schedule/canvas group-create/canvas node-move/memory patch/recommendation feedback preview/apply/rollback core exists and audit/rollback records survive reload through IndexedDB.
- Next target: wire real AI cards/tool writes through the command substrate instead of direct apply buttons, starting with the highest-risk existing UI consumer that still calls task/memory write APIs directly.
- Do not begin TASK-1856 command-center UI until at least one real non-test AI card/tool consumer is converted to the shared command substrate and proven in localhost/browser flow.
