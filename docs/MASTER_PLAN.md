...
- Start here in the next AI lane session before moving to TASK-1856. The task/subtask/lane preview/apply/rollback core exists and audit/rollback records survive reload through IndexedDB, but calendar/focus, canvas layout, and memory/feedback command families are still missing.
- Next target: extend typed commands and semantic duplicate decisions to calendar/focus blocks, canvas layout/group proposals, and memory/feedback events.
