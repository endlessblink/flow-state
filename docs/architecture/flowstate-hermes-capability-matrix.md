...
| Complete open-task inventory | Board, All Tasks, Today, Inbox; scoped task query | Yes: stable keyset snapshot, user/workspace scope, UUID dedupe, completion-record exclusion, typed freshness/completeness receipt | Yes: open/todo list and blank browse use the inventory boundary | RO | N/A | N/A | Source and packaged sidecar multi-page proof; exact total requires fresh + complete | Implemented for reliable assistant counts. P0 |
| Bounded done/due filters | Board, All Tasks, Today, Inbox; task filtering composables | Partial: status/due/25 max | Yes | RO | N/A | N/A | API list vs each UI filter | Keep as bounded filtering; never use as complete inventory evidence. P1 |
| Create task | All creation surfaces; `flowstate_task_lifecycle_v1` | Yes: deterministic ID, preview/apply, workspace scope, durable replay and canonical receipt | Pending H4 connector migration | REV | Yes/Yes | Canonical operation/change receipt | Receipt read-back + taskMutation reload | FlowState boundary done in TASK-1962; migrate Hermes tool before claiming end-to-end. P0 |
...
| Complete non-recurring | Task actions; `flowstate_complete_task_v1` via `POST /api/tasks/:id/complete` | Yes: dedicated preview/apply completion with approval digest, canonical receipt, and read-back proving `status`/`completedAt` | Yes | REV | Yes/Yes | Canonical receipt + change-log entry; recurring identity rejected with `recurring_task` | Receipt read-back + exact read | Done (TASK-1958). Recurring completion stays on `done-for-now`. |
| Soft-delete/archive | Context menus/trash; `flowstate_task_lifecycle_v1` | Yes: revision-bound preview/apply with tombstone proof | Pending H4 connector migration | DA | Yes/Yes | Canonical receipt + SOFT_DELETED audit | Receipt read-back + Trash/UI removal | FlowState boundary done; migrate Hermes tool. P0 |
| Restore | Trash; `flowstate_task_lifecycle_v1` | Yes: revision-bound preview/apply with recurrence conflict detection | Pending H4 connector migration | REV | Yes/Yes | Canonical receipt + RESTORED audit | Receipt read-back + Trash/Search | FlowState boundary done; migrate Hermes tool. P0 |
| Reopen non-recurring completion | Task lifecycle command | Yes: revision-bound preview/apply; recurrence rejected | Pending H4 connector migration | REV | Yes/Yes | Canonical receipt + status audit | Receipt read-back + taskMutation reload | Use only for non-recurring tasks; recurrence stays on done-for-now. P0 |
