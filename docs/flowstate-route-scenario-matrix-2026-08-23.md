...
- The fixture uses a committed disposable test identity and defaults to a local Supabase URL, with a service-role key supplied through the environment. That is suitable only for an isolated test stack; it is not evidence of the real user Google/Supabase path, and the harness needs explicit environment and identity assertions to prevent accidental execution against the wrong backend.
- Electron OAuth source review found two unproven edge cases: the loopback server resolves on the first HTTP request whether or not it contains a callback result, and the implicit-flow fallback expects URL-fragment tokens even though fragments are not sent in HTTP requests. Timeout, port collision, denial, repeated callback, and exchange-failure tests must be run against the packaged app, with a callback containing the expected code or error before the server is closed.
- Calendar credential storage review found an inconsistent persistence boundary: the debounced settings sync removes Google access/refresh tokens, but the explicit settings database mapper serializes the complete settings object. A settings save can therefore write provider credentials into shared `user_settings` JSON and another device can load them; this requires immediate schema/data audit, token rotation if any real credentials were stored, and one device-bound credential strategy.
- Updater source review found a silent failure state: failed-update markers are read to suppress a repeated notification, but the renderer converts the lack of an update event into `up-to-date`. The installed runtime can therefore remain stale while the UI reports no update; failed-download, suppressed-update, manual retry, and successful relaunch need separate visible outcomes.
...
- Timer source review found a direct disappearance path: a failed timer-session read is converted to `null`, and the follower poll then clears its local active session as if the timer had genuinely stopped. Network loss, expired auth, temporary RLS failure, and helper restart must preserve a distinct “unknown” state rather than showing an idle timer.
