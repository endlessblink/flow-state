...
The explicit sign-out/account-switch scenario is not closed: the visible account stores and offline task data are cleared, but Google provider credentials remain in the settings store unless the user separately disconnects Calendar. Mounted Calendar views also retain refresh/auto-sync eligibility because the connection flag remains true. This can make the next account appear connected to the previous account's Calendar and must be tested on the installed app after relaunch, not inferred from the cleared screen.
