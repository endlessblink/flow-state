...
| Local API delivery receipt | Renderer-to-main session/timer/auth calls return `{ ok: true }` when accepted by the main process, before the sidecar confirms receipt or listening; status also reports `running/listening` only when the persisted `enabled` flag is true, even when a child was started for a live session or timer snapshot. | Confirmed source contract; no authenticated delivery receipt captured. | Expose child generation and delivered-session state separately from requested state, wait for listening/acknowledgement where required, and test disabled-config, child-restart, delayed-start, and stale-token cases. |
| Timer read ambiguity | The timer database reader catches network/auth/database failures and returns `null`; the follower poll interprets `null` as a confirmed absence and clears the visible timer, while start-up cleanup can interpret the same failure as “nothing to clear” and create another active session. | Confirmed source control flow; no deliberate network-failure timer fixture executed. | Return an explicit unavailable/error result, preserve the last known session during read failure, block destructive start/cleanup on unknown state, and test follower, leader, restart, reconnect, and sidecar read-back behavior. |
...
| Task edit/cache split-brain | `updateTask` queues the remote write, then treats read-cache persistence as a required boundary; if that cache write throws, the method can surface an error without rolling back the optimistic task or cancelling the already-queued remote intent. A retry can create competing queued edits. | Confirmed source control-flow gap; no deliberate cache-failure runtime fixture executed. | Inject cache failure after queue acceptance, verify one durable intent and one user-visible outcome, then add rollback/cancellation or an explicit pending-save state. |
...
| Updater recovery UX | The updater has helpers to record and clear failed pending updates, but the active registration path only reads the failure marker and suppresses the matching update. The renderer then treats the absence of an update event as `up-to-date`, so a blocked update can look healthy with no recovery action. | Confirmed source path plus live suppression log; manual clear/retry and failed-download receipt are unproven. | Bind failure markers to a visible retry/repair action, record download/install failures, distinguish suppressed from up-to-date, and verify recovery from a deliberately failed update. |
