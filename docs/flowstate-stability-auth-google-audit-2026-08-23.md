...
| Timer read ambiguity | The timer database reader catches network/auth/database failures and returns `null`; the follower poll interprets `null` as a confirmed absence and clears the visible timer. A transient read failure can therefore make a still-active session disappear locally. | Confirmed source control flow; no deliberate network-failure timer fixture executed. | Return an explicit unavailable/error result, preserve the last known session during read failure, and test follower, leader, restart, reconnect, and sidecar read-back behavior. |
