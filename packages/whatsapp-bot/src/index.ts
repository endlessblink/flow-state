...
function validateConfig(): void {
  ...
  if (missing.length > 0) {
    console.warn(`[BOT] Missing env vars: ${missing.join(', ')}`)
    console.warn('[BOT] Bot will start but some features may not work')
  }

  if (ALLOWED_CHAT_IDS.length === 0) {
    console.warn('[BOT] ALLOWED_CHAT_IDS is empty — all messages will be rejected')
  }
}
...
app.post('/webhook/waha', async (req: Request, res: Response) => {
  ...
  if (!ALLOWED_CHAT_IDS.includes(chatId)) {
    console.log(`[BOT] Rejected message from unlisted chat: ${chatId}`)
    return
  }
  ...
  console.log(`[BOT] Processing message from ${chatId}: "${body.substring(0, 80)}${body.length > 80 ? '...' : ''}"`)
  ...
  try {
    ...
    console.log(`[BOT] Task created: "${task.title}" (${task.priority}) for ${chatId}`)
  } catch (error) {
...
app.listen(PORT, () => {
  console.log(`[BOT] FlowState WhatsApp Bot listening on port ${PORT}`)
  console.log(`[BOT] Webhook endpoint: POST http://localhost:${PORT}/webhook/waha`)
  console.log(`[BOT] Health check:     GET  http://localhost:${PORT}/health`)
  console.log(`[BOT] Allowed chats: ${ALLOWED_CHAT_IDS.length > 0 ? ALLOWED_CHAT_IDS.join(', ') : '(none)'}`)
})
