...
-- Current-main singular writes omitted order. Canonicalization adds only the
-- exact positional order and preserves every prior field and value.
DO $$
DECLARE
  v_legacy jsonb := '[{
    "id":"legacy-current-main",
    "parentTaskId":"1fc30000-0000-4000-8000-000000000101",
    "title":"Legacy current-main step",
  ...
  v_canonical jsonb;
BEGIN
  v_canonical := public.flowstate_h5_canonicalize_legacy_subtasks(
    v_legacy, '1fc30000-0000-4000-8000-000000000101'
  );
  IF NOT public.flowstate_h5_valid_subtasks(
       v_canonical, '1fc30000-0000-4000-8000-000000000101'
     )
     OR v_canonical #>> '{0,order}' IS DISTINCT FROM '0'
     OR (v_canonical->0) - 'order' IS DISTINCT FROM v_legacy->0 THEN
    RAISE EXCEPTION 'FAIL: current-main legacy canonicalization was lossy: %', v_canonical;
  END IF;
END;
$$;
...
BEGIN
  ...
  IF v_preview->>'previewDigest' IS DISTINCT FROM v_preview_again->>'previewDigest'
     ...
     OR (v_preview #> '{normalizedPayload,operations,2}') ?| ARRAY[
       'subtaskId', 'parentTaskId', 'completedPomodoros', 'isCompleted'
     ]
...
-- The canonical 10,001-row ceiling is checked on the post-operation array in
-- both preview and apply. Rejection cannot consume approval or write evidence.
DO $$
DECLARE
  v_rows jsonb;
  v_revision bigint;
  v_result jsonb;
  v_before_apply jsonb;
  v_operations jsonb := '[{"kind":"create","clientId":"approved-limit-step","title":"Approved boundary"}]'::jsonb;
  v_request_hash text;
  v_preview_digest text := repeat('d', 64);
  v_preview_expires_at timestamptz := clock_timestamp() + interval '15 minutes';
BEGIN
  SELECT jsonb_agg(jsonb_build_object(
    'id', 'limit-step-' || position,
    'parentTaskId', '1fc30000-0000-4000-8000-000000000103',
    'title', 'Limit step ' || position,
    'order', position
  ) ORDER BY position)
  INTO v_rows
  FROM generate_series(0, 10000) AS position;

  UPDATE public.tasks SET subtasks = v_rows
  WHERE id = '1fc30000-0000-4000-8000-000000000103';
  SELECT canonical_revision INTO v_revision FROM public.tasks
  WHERE id = '1fc30000-0000-4000-8000-000000000103';
  v_result := public.flowstate_subtask_batch_v1(
    'subtask-limit-preview', 'task-v1', 'local-api',
    '1fc30000-0000-4000-8000-000000000103', v_revision,
    '[{"kind":"create","clientId":"overflow-preview","title":"Must not fit"}]',
    true, NULL, NULL, NULL, NULL
  );
  IF v_result #>> '{error,code}' IS DISTINCT FROM 'subtask_limit_exceeded'
     OR jsonb_array_length((SELECT subtasks FROM public.tasks
                            WHERE id = '1fc30000-0000-4000-8000-000000000103')) <> 10001
     OR EXISTS (SELECT 1 FROM public.canonical_operation_previews
                WHERE operation_id = 'subtask-limit-preview') THEN
    RAISE EXCEPTION 'FAIL: post-state preview limit was not atomic: %', v_result;
  END IF;

  v_request_hash := encode(
    extensions.digest(
      convert_to(public.flowstate_canonical_json_text_v1(jsonb_build_object(
        'actorUserId', '1fc30000-0000-4000-8000-000000000001'::uuid,
        'contractVersion', 'task-v1',
        'source', 'local-api',
        'action', 'subtask_batch',
        'taskId', '1fc30000-0000-4000-8000-000000000103',
  ...
  INSERT INTO public.canonical_operation_previews (
    ...
    '1fc30000-0000-4000-8000-000000000001',
    'subtask-limit-apply', v_preview_digest, v_request_hash, v_preview_expires_at
  );
  SELECT subtasks INTO v_before_apply FROM public.tasks
  WHERE id = '1fc30000-0000-4000-8000-000000000103';

  v_result := public.flowstate_subtask_batch_v1(
    'subtask-limit-apply', 'task-v1', 'local-api',
    '1fc30000-0000-4000-8000-000000000103', v_revision,
    v_operations,
    false, v_preview_digest, v_preview_expires_at, NULL, v_request_hash
  );
  IF v_result #>> '{error,code}' IS DISTINCT FROM 'subtask_limit_exceeded'
     OR (SELECT subtasks FROM public.tasks
         WHERE id = '1fc30000-0000-4000-8000-000000000103') IS DISTINCT FROM v_before_apply
     OR EXISTS (SELECT 1 FROM public.canonical_operations
                WHERE operation_id = 'subtask-limit-apply')
     OR EXISTS (SELECT 1 FROM public.canonical_operation_previews
                WHERE operation_id = 'subtask-limit-apply' AND consumed_at IS NOT NULL) THEN
    RAISE EXCEPTION 'FAIL: post-state apply limit was not atomic: %', v_result;
  END IF;
