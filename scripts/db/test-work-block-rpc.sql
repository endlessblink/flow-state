...
('6b600000-0000-4000-8000-000000000109','6b600000-0000-4000-8000-000000000001','Legacy identity fixture','planned',null,false,'[{"id":"instance-6b600000-0000-4000-8000-000000000109-1720000000000","scheduledDate":"2026-07-16","scheduledTime":"08:00","duration":30}]','[]',true);
...
INSERT INTO work_block_results SELECT 'legacy_move_preview', public.flowstate_work_block_v1(
  'work-block-legacy-move','work-block-v1','local-api','6b600000-0000-4000-8000-000000000109',1,0,
  '{"action":"move","workBlockId":"instance-6b600000-0000-4000-8000-000000000109-1720000000000","scheduledDate":"2026-07-17","scheduledTime":"09:15","timezone":"Asia/Jerusalem"}',true);
INSERT INTO work_block_results SELECT 'legacy_move_apply', public.flowstate_work_block_v1(
  'work-block-legacy-move','work-block-v1','local-api','6b600000-0000-4000-8000-000000000109',1,0,
  '{"action":"move","workBlockId":"instance-6b600000-0000-4000-8000-000000000109-1720000000000","scheduledDate":"2026-07-17","scheduledTime":"09:15","timezone":"Asia/Jerusalem"}',false,
  preview.payload->>'previewDigest',(preview.payload->>'previewExpiresAt')::timestamptz)
FROM work_block_results preview WHERE key='legacy_move_preview';
DO $$
DECLARE applied jsonb := (SELECT payload FROM work_block_results WHERE key='legacy_move_apply');
BEGIN
  IF applied->>'result' <> 'committed'
     OR applied#>>'{receipt,workBlockId}' <> 'instance-6b600000-0000-4000-8000-000000000109-1720000000000'
     OR applied#>>'{receipt,readBack,workBlock,canonicalRevision}' <> '1'
     OR applied#>>'{receipt,readBack,workBlock,taskId}' <> '6b600000-0000-4000-8000-000000000109'
     OR applied#>>'{receipt,readBack,workBlock,timezone}' <> 'Asia/Jerusalem' THEN
    RAISE EXCEPTION 'FAIL: shipped legacy work-block identity was not upgraded transactionally: %', applied;
  END IF;
END $$;

INSERT INTO work_block_results SELECT 'legacy_remove_preview', public.flowstate_work_block_v1(
  'work-block-legacy-remove','work-block-v1','local-api','6b600000-0000-4000-8000-000000000109',2,1,
  '{"action":"remove","workBlockId":"instance-6b600000-0000-4000-8000-000000000109-1720000000000","timezone":"Asia/Jerusalem"}',true);
INSERT INTO work_block_results SELECT 'legacy_remove_apply', public.flowstate_work_block_v1(
  'work-block-legacy-remove','work-block-v1','local-api','6b600000-0000-4000-8000-000000000109',2,1,
  '{"action":"remove","workBlockId":"instance-6b600000-0000-4000-8000-000000000109-1720000000000","timezone":"Asia/Jerusalem"}',false,
  preview.payload->>'previewDigest',(preview.payload->>'previewExpiresAt')::timestamptz)
FROM work_block_results preview WHERE key='legacy_remove_preview';
DO $$
BEGIN
  BEGIN
    UPDATE public.tasks
    SET instances='[{"id":"instance-6b600000-0000-4000-8000-000000000109-1720000000000","scheduledDate":"2026-07-21","scheduledTime":"11:00","duration":25}]'::jsonb
    WHERE id='6b600000-0000-4000-8000-000000000109';
    RAISE EXCEPTION 'FAIL: retired legacy work-block ID was recreated';
  EXCEPTION WHEN check_violation THEN
    NULL;
  END;
END $$;
INSERT INTO work_block_results SELECT 'legacy_remove_replay', public.flowstate_work_block_v1(
  'work-block-legacy-remove','work-block-v1','local-api','6b600000-0000-4000-8000-000000000109',2,1,
  '{"action":"remove","workBlockId":"instance-6b600000-0000-4000-8000-000000000109-1720000000000","timezone":"Asia/Jerusalem"}',false,
  preview.payload->>'previewDigest',(preview.payload->>'previewExpiresAt')::timestamptz)
FROM work_block_results preview WHERE key='legacy_remove_preview';
DO $$
BEGIN
  IF (SELECT payload#>>'{receipt,replayed}' FROM work_block_results WHERE key='legacy_remove_replay') <> 'true'
     OR EXISTS (
       SELECT 1 FROM public.tasks task
       CROSS JOIN LATERAL pg_catalog.jsonb_array_elements(task.instances) item(value)
       WHERE task.id='6b600000-0000-4000-8000-000000000109'
         AND item.value->>'id'='instance-6b600000-0000-4000-8000-000000000109-1720000000000'
     )
     OR NOT EXISTS (
       SELECT 1 FROM public.canonical_legacy_work_block_ids claim
       WHERE claim.work_block_id='instance-6b600000-0000-4000-8000-000000000109-1720000000000'
         AND claim.retired_at IS NOT NULL
     ) THEN
    RAISE EXCEPTION 'FAIL: legacy remove/recreate/replay identity reservation diverged';
  END IF;
END $$;
...
DECLARE p jsonb := (SELECT payload FROM work_block_results WHERE key='create_preview');
        ...
        h text;
BEGIN
  h := encode(extensions.digest(convert_to(public.flowstate_canonical_json_text_v1(a#>'{receipt,readBack}'),'UTF8'),'sha256'),'hex');
  IF p#>>'{preview,interval,after,localStart}' <> '2026-07-16T10:00'
     ...
     OR ((r - 'status') #- '{receipt,replayed}' #- '{receipt,status}')
        IS DISTINCT FROM ((a - 'status') #- '{receipt,replayed}' #- '{receipt,status}')
...
INSERT INTO work_block_results SELECT 'resize_preview', public.flowstate_work_block_v1(
  ...
  '{"action":"resize","workBlockId":"6b600000-0000-4000-8000-000000000201","duration":90,"timezone":"UTC","finishBy":"2026-07-17T11:00"}',true);
INSERT INTO work_block_results SELECT 'resize_apply', public.flowstate_work_block_v1(
  'work-block-resize','work-block-v1','local-api','6b600000-0000-4000-8000-000000000101',3,2,
  '{"action":"resize","workBlockId":"6b600000-0000-4000-8000-000000000201","duration":90,"timezone":"UTC","finishBy":"2026-07-17T11:00"}',false,payload->>'previewDigest',(payload->>'previewExpiresAt')::timestamptz)
...
INSERT INTO work_block_results SELECT 'remove_preview', public.flowstate_work_block_v1(
  ...
  '{"action":"remove","workBlockId":"6b600000-0000-4000-8000-000000000201","timezone":"UTC"}',true);
INSERT INTO work_block_results SELECT 'remove_apply', public.flowstate_work_block_v1(
  'work-block-remove','work-block-v1','local-api','6b600000-0000-4000-8000-000000000101',4,3,
  '{"action":"remove","workBlockId":"6b600000-0000-4000-8000-000000000201","timezone":"UTC"}',false,payload->>'previewDigest',(payload->>'previewExpiresAt')::timestamptz)
...
INSERT INTO work_block_results SELECT 'malformed_existing',public.flowstate_work_block_v1('work-block-malformed','work-block-v1','local-api','6b600000-0000-4000-8000-000000000105',1,1,'{"action":"remove","workBlockId":"6b600000-0000-4000-8000-000000000209","timezone":"UTC"}',true);
DO $$ BEGIN
...
-- Recurrence-heavy scopes must stay bounded and preserve every legacy instance.
