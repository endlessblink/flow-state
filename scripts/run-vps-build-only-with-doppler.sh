...
if ! node_is_supported && [[ -x /tmp/flowstate-node22/bin/node ]]; then
  export PATH="/tmp/flowstate-node22/bin:${PATH}"
fi
node_is_supported || { echo "Node 22.13+ is required for the release build" >&2; exit 1; }
