...
ssh -T -i "$SSH_KEY" "${VPS_USER}@${VPS_HOST}" \
  "set -e; export PATH='${NODE_BIN}:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin'; mkdir -p \"$(dirname '${REMOTE_REPO}')\"; if [[ ! -d '${REMOTE_REPO}/.git' ]]; then git clone --branch main --single-branch https://github.com/endlessblink/flow-state.git '${REMOTE_REPO}'; else git -C '${REMOTE_REPO}' fetch origin main:refs/remotes/origin/main; git -C '${REMOTE_REPO}' reset --hard origin/main; rm -rf '${REMOTE_REPO}/node_modules' '${REMOTE_REPO}/dist' '${REMOTE_REPO}/release'; git -C '${REMOTE_REPO}' clean -ffdx; fi; test \"\$(grep -m1 '\"version\": \"' '${REMOTE_REPO}/package.json' | cut -d '\"' -f4)\" = '${RELEASE_VERSION}'; cd '${REMOTE_REPO}'; '${NODE_BIN}/npm' ci --include=dev --ignore-scripts --no-audit --no-fund" < /dev/null
...
ssh -T -i "$SSH_KEY" "${VPS_USER}@${VPS_HOST}" \
  "set -e; set -a; . '${REMOTE_SECRET_FILE}'; set +a; export PATH='${NODE_BIN}:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin'; cd '${REMOTE_REPO}'; /usr/bin/doppler run -- env npm_config_production=false '${NODE_BIN}/npm' run electron:validate-env; /usr/bin/doppler run -- env npm_config_production=false '${NODE_BIN}/npm' run electron:build:locked; ELECTRON_BUILD=false /usr/bin/doppler run -- env npm_config_production=false '${NODE_BIN}/npm' run build; '${NODE_BIN}/node' scripts/generate-flowstate-release-receipt.cjs --version '${RELEASE_VERSION}' --source-commit \"\$(git rev-parse HEAD)\" --pwa-dir '${REMOTE_REPO}/dist' --electron-dir '${REMOTE_REPO}/release' --output '${REMOTE_REPO}/release/flowstate-release-receipt.json'"
