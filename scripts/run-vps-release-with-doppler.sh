...
ssh -T -i "$SSH_KEY" "${VPS_USER}@${VPS_HOST}" \
  "set -a; . /etc/flowstate/doppler-release.env; set +a; export PATH='${NODE_BIN}:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin'; cd '${REMOTE_REPO}'; git fetch origin '${RELEASE_COMMIT}'; git reset --hard '${RELEASE_COMMIT}'; test -z \"\$(git status --porcelain --untracked-files=all)\"; test \"\$(git rev-parse HEAD)\" = '${RELEASE_COMMIT}'; git cat-file -e '${RELEASE_COMMIT}^{commit}'; export DOPPLER_PROJECT=flow-state DOPPLER_CONFIG=prd; /usr/bin/doppler run --project flow-state --config prd -- '${NODE_BIN}/npm' run electron:build:locked"
