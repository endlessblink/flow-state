...
const { canonicalHash, validateCanonicalReceipt } = require('./canonical-receipt.cjs')
...
function normalizeCommand(command, workBlockRevision) {
  ...
  if (!nonEmptyString(command.workBlockId)
    || command.workBlockId.length > 200
    || !Number.isSafeInteger(workBlockRevision)
    || workBlockRevision < 0) return null
  const workBlockId = UUID_RE.test(command.workBlockId)
    ? command.workBlockId.toLowerCase()
    : command.workBlockId
  ...
  if (command.action === 'resize') {
    if (!exactKeys(command, new Set(['action', 'workBlockId', 'duration', 'timezone', 'finishBy']), [
      'action', 'workBlockId', 'duration', 'timezone',
    ]) || !validDuration(command.duration) || !validTimezone(command.timezone)) return null
    const normalized = { action: 'resize', workBlockId, duration: command.duration, timezone: command.timezone }
    return normalizeFinishBy(command, normalized) ? normalized : null
  }
  return exactKeys(command, new Set(['action', 'workBlockId', 'timezone']), ['action', 'workBlockId', 'timezone'])
    && validTimezone(command.timezone)
    ? { action: 'remove', workBlockId, timezone: command.timezone }
    : null
}
...
function validateRequest(taskId, body) {
  ...
  if (!preview && (
    !DIGEST_RE.test(body.previewDigest || '')
    || !timestamp(body.previewExpiresAt)
    || !DIGEST_RE.test(body.requestHash || '')
  )) {
    return {
      error: errorResult(
        400,
...
function validCommitted(data, taskId, body, command, workspaceId, expectedHash) {
  ...
  if (!object(data)
    || data.ok !== true
    || !['committed', 'replayed'].includes(data.status)
    || data.result !== 'committed'
    || data.requestHash !== expectedHash
    || !object(data.receipt)
    || data.receipt.status !== data.status
    || data.receipt.requestHash !== data.requestHash
    || data.receipt.workBlockId !== workBlockId(command)
    || !object(data.receipt.readBack)
    || data.receipt.canonicalRevision !== data.receipt.readBack.canonicalRevision
    || data.receipt.canonicalUpdatedAt !== data.receipt.readBack.canonicalUpdatedAt) return false
  return validateCanonicalReceipt(data.receipt, {
    ...
    validateReadBack: readBack => validCommittedReadBack(
      readBack, taskId, body, command, workspaceId,
    ),
  }).ok
}
...
async function executeCanonicalWorkBlock(context, taskId, body, notifyTaskMutation) {
  ...
  if (!validated.preview && body.requestHash !== expectedHash) {
    return errorResult(409, 'request_hash_mismatch', 'The approval hash does not match this request')
  }
