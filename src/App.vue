...
onMounted(async () => {
  ...
  isElectronApp.value = !!(window as unknown as { electronAPI?: { isElectron?: boolean; getHomeOverride?: () => Promise<{ home: string; pinnedTo: string } | null> } }).electronAPI?.isElectron
  ...
  if (isElectronApp.value) {
    void (async () => {
      try {
        const override = await (window as unknown as { electronAPI?: { isElectron?: boolean; getHomeOverride?: () => Promise<{ home: string; pinnedTo: string } | null> } }).electronAPI?.getHomeOverride?.()
        if (!override) return
