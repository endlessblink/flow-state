...
watch(() => (props.data as any)?.name, (newName) => {
  sectionName.value = newName
})

const updateName = () => {
  if (sectionName.value !== (props.data as any)?.name) {
...
const handleContextMenu = (event: MouseEvent) => {
  emit('contextMenu', event, props.data as any)
}
