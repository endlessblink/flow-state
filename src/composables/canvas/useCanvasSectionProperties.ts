...
export function useCanvasSectionProperties(deps: SectionPropertiesDeps) {
  ...
  // ================================================================
  const getSingleSectionProperties = (
    ...
    if (matchedDay !== undefined) {
      ...
      updates.dueDate = toDateString(
        getDayGroupDate(matchedDay, today, hasTodayOrTomorrow),
      );
    }
      ...
      switch (keyword.category) {
        case "date":
          switch (keyword.value) {
            ...
            case "this weekend": {
              ...
              sat.setDate(
                today.getDate() + ((6 - today.getDay() + 7) % 7 || 7),
              );
        ...
        case "duration":
          updates.estimatedDuration =
            DURATION_DEFAULTS[keyword.value as DurationCategory] ?? 0;
          break;
      }
    }
  ...
  const getSectionProperties = (
    ...
    for (let i = chain.length - 1; i >= 0; i--) {
      ...
      const props = getSingleSectionProperties(
        group as CanvasSection,
        allGroups,
      );
