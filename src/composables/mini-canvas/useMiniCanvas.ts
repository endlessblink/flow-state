...
export function useMiniCanvas(taskId: () => string | null) {
  ...
  const nodes = computed<Node[]>(() => {
    ...
    const totalChildren =
      (t.subtasks || []).length + (t.planningNotes || []).length;
    let childIndex = 0;

    // Subtask nodes
    for (const subtask of t.subtasks || []) {
      const pos =
        subtask.canvasPosition || radialPosition(childIndex, totalChildren);
    ...
    for (const note of t.planningNotes || []) {
      const pos =
        note.canvasPosition || radialPosition(childIndex, totalChildren);
  ...
  const createConnectedSubtask = async (
    ...
    const sourcePos = nodes.value.find((n) => n.id === sourceId)?.position || {
      x: 0,
      y: 0,
    };
