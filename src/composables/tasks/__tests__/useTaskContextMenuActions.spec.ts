...
describe('useTaskContextMenuActions toggleDone canonical resolution', () => {
  ...
  it('USER REPRO: waits through a multi-tick Catalog refresh before failing completion', async () => {
    const visibleCatalogTask = {
      id: 'task-1',
      status: 'todo',
      title: 'Visible during refresh',
    } as Task
    const canonicalTask = { ...visibleCatalogTask, status: 'todo' } as Task
    getTask
      .mockReturnValueOnce(undefined)
  ...
  it('USER REPRO: retries when the undo wrapper loses the target after the first lookup', async () => {
    const visibleCatalogTask = {
      id: 'task-1',
      status: 'todo',
      title: 'Visible during mutation refresh',
    } as Task
