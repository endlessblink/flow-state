...
export function useAIChat() {
  ...
  function fallbackClarificationReason(evidence: string, lang: 'he' | 'en'): string {
    const clipped = evidence.replace(/\s+/g, ' ').trim().slice(0, 120)
    if (!clipped) {
      return lang === 'he'
        ? 'הבחירה נשענת על תשובת ההבהרה, אבל הקשר חסר עדיין מסומן'
        : 'the choice follows your clarification, while missing context stays marked'
    }
    return lang === 'he'
      ? `לפי תשובת ההבהרה שלך: ${clipped}`
  ...
  const BROAD_TASK_RECOMMENDATION_RE = /(what should i do|what to do|prioriti[sz]e|priority|important|overwhelm|plan my day|plan today|focus on|next task|next action|מה לעשות|מה כדאי|לתעדף|סדר עדיפויות|חשוב|להתמקד|מוצף|לתכנן|המשימה הבאה)/i

  function shouldAskBroadTaskClarification(content: string, routed: RoutedIntent, hasTaskList: boolean): boolean {
  ...
  function broadTaskClarificationMemoryKey(routed: RoutedIntent): string {
    return `workflow:task_answer:${routed.responseMode || 'general'}`
  }

  function hasRecentClarificationDecision(events: AIClarificationEvent[], now = Date.now()): boolean {
  ...
  function buildBroadTaskClarification(
    content: string,
    routed: RoutedIntent,
  ...
  async function sendMessageDeterministic(
    ...
    try {
      ...
      const clarificationContextForFormatter = clarificationContinuationEvidence
        ...
        : ''
      const formatterFallbackOptions: FormatterFallbackOptions = {
      ...
      if (!isClarificationContinuation && shouldAskBroadTaskClarification(content, routed, hasTaskList)) {
        ...
        const broadClarification = buildBroadTaskClarification(content, routed, toolResults, outputLanguage, broadClarificationEvents)
        if (broadClarification) {
          ...
          store.updateActivityEvent(phaseActivityId, {
            metadata: {
              startedAt,
              elapsedMs: Date.now() - startedAt,
              pathType: broadClarification.pathType,
              source: broadClarification.debug?.retrieval?.source,
              reason: broadClarification.debug?.reason,
              timedOut: false,
            },
          })
          if (lastMsg && lastMsg.isStreaming) {
          ...
          void (async () => {
            try {
              ...
              await db.recordAIClarificationEvent({
                entityKey: broadClarification.memoryKey,
                entityType: broadClarification.question.entityType ?? 'workflow',
                displayName: broadClarification.question.entityId ?? broadClarification.memoryKey,
                questionId: broadClarification.question.id,
                eventType: 'asked',
                question: broadClarification.question.question,
                sourceMessageId: lastMsg?.id,
                coverageScoreAtTime: broadClarification.coverage?.score,
                uncertaintyDimensions: broadClarification.coverage?.missing,
                pathType: broadClarification.pathType,
                contextSnapshot: {
                  candidateTaskIds: broadClarification.candidateTaskIds,
                  coverage: broadClarification.coverage,
                  retrieval: broadClarification.debug?.retrieval,
                },
              })
            } catch (eventErr) {
...
- Never repeat the same reasoning template across multiple tasks.`
      ...
      try {
        ...
        formattedResponse = buildFormatterFallback(toolResults, routed.language, routed.responseMode, formatterFallbackOptions)
      }
      ...
      if (!cardData && cardsInstruction && hasTaskList) {
        const fallbackResponse = buildFormatterFallback(toolResults, routed.language, routed.responseMode, formatterFallbackOptions)
