...
export const useCouchDBSync = () => {
  ...
  // Setup local cross-tab synchronization using PouchDB changes feed
  const setupLocalCrossTabSync = (localDB: PouchDB.Database) => {
    console.log('🔄 Setting up local cross-tab synchronization')
    ...
    changesHandler.on('error', (error) => {
      console.error('❌ Local changes feed error:', error)
      syncStatus.value = 'error'
    })

    syncStatus.value = 'complete'
    console.log('✅ Local cross-tab sync initialized')
