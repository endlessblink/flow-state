/**
 * Power Keywords - Task Property Auto-Assignment System
 *
 * Provides centralized logic for detecting power keywords in group names.
 * Groups with special keywords automatically become "power groups"
 ...
 * - Priority: "high priority", "urgent", "low priority", etc.
 * - Status: "done", "completed", "in progress", "backlog", etc.
 * - Duration: "quick task", "short task", "medium task", "long task"
 * - Day of Week: "monday", "tuesday", etc.
 *
 * @example
 * ```ts
 * import { detectPowerKeyword, isPowerGroup } from '@/composables/usePowerKeywords'
 *
 * const result = detectPowerKeyword('Today Tasks')
 * // { keyword: 'today', category: 'date', value: 'today', displayName: 'Today' }
 *
 * isPowerGroup('High Priority') // true
 * isPowerGroup('Random Group')  // false
 * ```
 *
 * NOTE: This file was renamed from useTaskSmartGroups.ts for clarity.
...
export function applySmartGroupProperties(task: Task, smartGroupType: SmartGroupType): Partial<Task> {
  ...
  console.log(`[Power Keywords] Applied ${smartGroupType} properties to task "${task.title}":`, {
