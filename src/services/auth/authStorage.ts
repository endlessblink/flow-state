...
let pendingElectronPkce: { key: string; preserving: boolean } | null = null

/** Keep auth-js session maintenance from deleting an in-progress desktop OAuth verifier. */
export function beginElectronPkceAttempt(storageKey: string): () => Promise<void> {
    if (!hasElectronRuntimeHint()) throw new Error('Electron OAuth requires the Electron runtime')
    if (pendingElectronPkce) throw new Error('An Electron sign-in attempt is already in progress')
    const storage = createLazyAuthStorage()
    if (!storage) throw new Error('Electron OAuth storage is unavailable')
    const attempt = { key: `${storageKey}-code-verifier`, preserving: true }
    pendingElectronPkce = attempt
    let cleanup: Promise<void> | undefined
    return () => {
        if (!cleanup) {
            attempt.preserving = false
            // Retain ownership until durable deletion settles; an old finally must
            // never remove the verifier generated by a newer attempt.
            cleanup = storage.removeItem(attempt.key).finally(() => {
                if (pendingElectronPkce === attempt) pendingElectronPkce = null
            })
        }
        return cleanup
    }
...
export function createLazyAuthStorage(): AsyncAuthStorage | null {
    ...
    return {
        ...
        removeItem: async (key: string): Promise<void> => {
            if (pendingElectronPkce?.preserving && pendingElectronPkce.key === key) return
            const electronRuntime = hasElectronRuntimeHint()
            const api = electronBridge() || await waitForElectronBridge()
            if (api) {
