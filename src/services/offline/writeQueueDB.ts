...
class WriteQueueDatabase extends Dexie {
  ...
  canonicalReceiptsV2!: Table<
    StoredCanonicalTaskPatchReceipt,
    [string, string]
  >;
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      operations:
        "++id, status, [entityType+entityId], createdAt, nextRetryAt, userId",
    ...
    this.version(2).stores({
      operations:
        "++id, status, [entityType+entityId], createdAt, nextRetryAt, userId, workspaceId",
      conflicts: "++id, [operation.entityType+operation.entityId], detectedAt",
      metadata: "key",
    });
    ...
    this.version(4).stores({
      ...
      metadata: "key",
      canonicalReceipts:
        "operationId, [scopeKey+entityId], scopeKey, entityId, changeSequence",
    });
    ...
    this.version(5)
      .stores({
        operations:
          "++id, status, [entityType+entityId], createdAt, nextRetryAt, userId, workspaceId",
        conflicts:
          "++id, [operation.entityType+operation.entityId], detectedAt",
        metadata: "key",
        canonicalReceipts:
          "operationId, [scopeKey+entityId], scopeKey, entityId, changeSequence",
        canonicalReceiptsV2:
          "[scopeKey+operationId], operationId, [scopeKey+entityId], scopeKey, entityId, changeSequence",
        canonicalCheckpoints:
          "[scopeKey+entityId], scopeKey, entityId, canonicalRevision",
      })
      .upgrade(async (transaction) => {
        const legacy = await transaction
          .table<StoredCanonicalTaskPatchReceipt>("canonicalReceipts")
          .toArray();
        if (legacy.length > 0) {
          await transaction
            .table<StoredCanonicalTaskPatchReceipt>("canonicalReceiptsV2")
            .bulkPut(legacy);
          ...
          for (const receipt of legacy) {
            ...
            if (
              !current ||
              receipt.canonicalRevision > current.canonicalRevision
            ) {
          ...
          await transaction
            .table<CanonicalRevisionCheckpoint>("canonicalCheckpoints")
...
export async function enqueueOperation(
  ...
  const cloned = JSON.parse(
    JSON.stringify(toRaw(fullOperation)),
  ) as WriteOperation;
...
/** Requeue the one legacy permanent error made repairable by the current
...
export async function repairStaleCanonicalPreviewFailures(): Promise<number> {
  ...
  for (const operation of candidates) {
    ...
    if (!error.includes('invalid_canonical_preview') && !error.includes('invalid_persisted_canonical_preview')) continue
...
export async function getCanonicalReceipt(
  ...
  const stored = await getWriteQueueDB().canonicalReceiptsV2.get([
    canonicalScopeKey(userId, workspaceId),
    operationId,
  ]);
...
export async function getLatestCanonicalReceiptForEntity(
  ...
  const receipts = await getWriteQueueDB()
    .canonicalReceiptsV2.where("[scopeKey+entityId]")
    .equals([canonicalScopeKey(userId, workspaceId), entityId])
    .toArray();
  const stored = receipts.sort(
    (left, right) =>
...
export async function hasEarlierUnresolvedOperation(
  operation: WriteOperation,
): Promise<boolean> {
  const operationId = operation.id;
  if (!operationId) return false;
  const operations = await getOperationsForEntity(
  ...
  return operations.some(
    (candidate) =>
      ...
      (candidate.createdAt < operation.createdAt ||
        (candidate.createdAt === operation.createdAt &&
          Number(candidate.id) < operationId)),
  );
}
...
export async function getFailedCount(): Promise<number> {
  ...
  return db.operations.where("status").equals("failed").count();
}
 ...
 */
export async function purgeStaleOperations(
 ...
 */
export async function resolveConflictAcceptServer(
  conflictId: number,
): Promise<void> {
  const db = getWriteQueueDB();
...
export async function getStats(): Promise<{
  ...
  const [pending, syncing, failed, completed, conflicts, total] =
    await Promise.all([
