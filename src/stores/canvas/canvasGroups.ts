...
} from "@/utils/canvas/spatialContainment";
...
import {
  ...
  isMigratableDayGroup,
} from "@/utils/canvas/legacyGroupId";
...
export const useCanvasGroups = (
  ...
  const setGroups = (newGroups: CanvasGroup[], forceEmpty = false) => {
    if (newGroups.length === 0 && _rawGroups.value.length > 0 && !forceEmpty) {
      console.error(
        "❌ [CANVAS] Refusing to overwrite existing groups with empty array",
      );
      return;
    }
    if (import.meta.env.DEV) {
      ...
      if (removed.length > 0) {
        console.log(
          `[SETGROUPS-DIAG] replacing drops ${removed.length} group(s): ${removed.map((g) => `${g.name}/${g.id.slice(0, 8)}`).join(", ")}`,
  ...
  const createGroup = async (
    groupData: Omit<CanvasGroup, "id"> | CanvasGroup,
  ) => {
    applySmartGroupNormalizations(groupData);
    const newGroup: CanvasGroup = {
      ...groupData,
      id:
        "id" in groupData && groupData.id ? groupData.id : crypto.randomUUID(), // TASK-1183: Use proper UUID for Supabase compatibility
      isVisible: true,
      isCollapsed: false,
  ...
  const updateGroup = async (id: string, updates: Partial<CanvasGroup>) => {
    ...
    if (index !== -1) {
      ...
      if (import.meta.env.DEV) {
        if (
          "parentGroupId" in updates &&
          updates.parentGroupId !== group.parentGroupId
        ) {
          console.log(
            `📍[GROUP-PARENT-WRITE] Group ${id.slice(0, 8)}... (${group.name}) parentGroupId: "${group.parentGroupId ?? "none"}" → "${updates.parentGroupId ?? "none"}"`,
          );
        }
        ...
        if ("position" in updates && updates.position) {
          ...
          if (oldPos?.x !== newPos?.x || oldPos?.y !== newPos?.y) {
            console.log(
              `📍[GROUP-POS-WRITE] Group "${group.name?.slice(0, 20)}" (${id.slice(0, 8)})`,
              {
                before: oldPos
                  ? { x: Math.round(oldPos.x), y: Math.round(oldPos.y) }
                  : null,
  ...
  const deleteGroup = async (id: string) => {
    ...
    if (index !== -1) {
      ...
      const childTasks =
        taskStoreRef.value?.tasks.filter((t) => t.parentId === id) ?? [];
          ...
          await Promise.all(
            childTasks.map((t) =>
              taskStore.updateTask(
                t.id,
                { parentId: undefined },
                "GROUP_DELETE" as Parameters<typeof taskStore.updateTask>[2],
              ),
            ),
          );
        } catch (err) {
          console.error(
            "[BUG-1510] Failed to clear parentId on child tasks before group delete:",
            err,
          );
        }
      }
  ...
  const aggregatedTaskCountByGroupId = computed(() => {
    ...
    if (!taskStoreRef.value || !taskStoreRef.value.tasks)
  ...
  const getTasksInSection = (groupId: string, tasks?: Task[]): Task[] => {
    ...
    const groupAbsolutePos = getGroupAbsolutePosition(
      groupId,
      _rawGroups.value,
    );
  ...
  const updateGroupFromSync = (groupId: string, data: Partial<CanvasGroup>) => {
    ...
    if (!data || typeof data !== "object") {
      console.warn(
        `[GROUP-SYNC] Ignoring invalid data for group ${groupId}:`,
        data,
      );
      return;
    }
    ...
    if (pendingGroupWrites.has(groupId)) {
      if (import.meta.env.DEV) {
        console.log(
          `[GROUP-SYNC] Skipping sync for group ${groupId.slice(0, 8)}... - pending local write`,
        );
      }
      return;
    }
    ...
    try {
      ...
      if (index !== -1) {
        ...
        if (localVersion > incomingVersion) {
          if (import.meta.env.DEV) {
            console.log(
              `[GROUP-SYNC] Skipping older version for group ${groupId.slice(0, 8)}... (local v${localVersion} > incoming v${incomingVersion})`,
            );
          }
          return;
        }
        ...
        if (
          ...
          if (localTime > incomingTime) {
            if (import.meta.env.DEV) {
              console.log(
                `[GROUP-SYNC] Skipping older timestamp for group ${groupId.slice(0, 8)}... (local newer by ${localTime - incomingTime}ms)`,
              );
            }
            return;
          }
        ...
        if (
          ...
          if (import.meta.env.DEV) {
            console.log(
              `[GROUP-SYNC] Dropping equal-version geometry for group ${groupId.slice(0, 8)}... (local v${localVersion} owns its geometry)`,
            );
          }
  ...
  const migrateLegacyGroupIds = async (
    userId: string,
    ...
    try {
      const snapshot = {
        ...
        taskParents: (taskStore.rawTasks as Task[]).map((t) => ({
          id: t.id,
          parentId: t.parentId ?? null,
        })),
      };
      localStorage.setItem(
        "flowstate:legacy-group-migration-backup",
        JSON.stringify(snapshot),
      );
      console.log(
        `💾 [LEGACY-MIGRATE] Backed up ${snapshot.groups.length} groups before migrating ${legacy.length} legacy group(s) (localStorage: flowstate:legacy-group-migration-backup)`,
      );
    } catch (e) {
      console.error(
        "[LEGACY-MIGRATE] Backup failed — aborting migration to be safe:",
        e,
      );
      return { migrated: 0 };
    }
    ...
    cacheGroups([..._rawGroups.value]);
    console.log(
      `✅ [LEGACY-MIGRATE] Migrated ${legacy.length} legacy group(s) to UUIDs`,
    );
