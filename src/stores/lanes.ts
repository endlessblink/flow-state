...
export const useLaneStore = defineStore('lanes', () => {
    ...
    const lanes = computed(() => _rawLanes.value.filter(l => {
        if (!l.id) return false
        if ((l as Record<string, unknown>).is_deleted === true || (l as Record<string, unknown>).isDeleted === true) return false
        if (!l.name || typeof l.name !== 'string' || l.name.trim() === '') return false
        return true
    }))
    ...
    const { fetchLanes, saveLane, deleteLane: deleteLaneRemote } = useSupabaseDatabase()
        ...
        loadInFlightPromise = (async () => {
            ...
            try {
                ...
                if (!authStore.isAuthenticated) {
                    ...
                    console.log(`👤 [GUEST-MODE] Loaded ${_rawLanes.value.length} lanes from localStorage`)
                    return
                }
    ...
    const enqueueLaneOp = async (operation: 'create' | 'update' | 'delete', lane: Lane | { id: string }) => {
        try {
            ...
            if (operation !== 'delete') {
                ...
                payload = JSON.parse(JSON.stringify(toSupabaseLane(lane as Lane, userId)))
            }
            ...
            console.warn(`[SYNC-QUEUE] Failed to queue lane ${operation}:`, queueError)
        }
    }
    ...
    const createLane = async (laneData: Partial<Lane>) => {
        ...
        try {
            ...
            const newLane: Lane = {
                ...
                updatedAt: new Date(),
                ...laneData,
                workspaceId: laneData.workspaceId !== undefined ? laneData.workspaceId : activeWorkspaceId,
            } as Lane
        ...
        try {
            ...
            for (let i = 0; i < affected.length; i += BATCH) {
                ...
                await Promise.all(batch.map(t =>
                    taskStore.updateTask(t.id, { laneId: null }).catch(err =>
                        console.error(`[DELETE-LANE] Failed to clear lane on task ${t.id}:`, err)
                    )
                ))
            }
            ...
            console.error('[DELETE-LANE] Delete failed, rolling back local state:', e)
    ...
    watch(lanes, () => {
        if (manualOperationInProgress.value || isLoading.value || syncUpdateInProgress) return
        if (saveTimeout) clearTimeout(saveTimeout)
        saveTimeout = setTimeout(() => { saveLanesToLocalStorage() }, 1000)
    }, { deep: true })
