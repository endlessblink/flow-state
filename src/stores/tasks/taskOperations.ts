...
const isRealTaskTitle = (title: unknown): title is string =>
  typeof title === "string" &&
  title.trim().length > 0 &&
  title.trim() !== FALLBACK_TASK_TITLE;

const hasTaskTitle = (title: unknown): title is string =>
  typeof title === "string" && title.trim().length > 0;

const shouldKeepPermanentDeleteLocallyOnRemoteFailure = (
  error: unknown,
): boolean => {
  const status =
...
// =============================================================================
export type GeometryWriteSource =
  | "DRAG"
  | "RECONCILE"
  | "USER"
  | "SYNC"
...
const FORBIDDEN_GEOMETRY_SOURCES = new Set<GeometryWriteSource>([
  "SYNC",
  "SMART-GROUP",
]);
...
const sanitizeGeometryUpdates = (
  ...
  const attemptedGeometryChange =
    ("parentId" in sanitized && sanitized.parentId !== task.parentId) ||
    ("canvasPosition" in sanitized &&
      sanitized.canvasPosition !== undefined &&
      (task.canvasPosition?.x !== sanitized.canvasPosition?.x ||
        task.canvasPosition?.y !== sanitized.canvasPosition?.y));

  const blocked =
    FORBIDDEN_GEOMETRY_SOURCES.has(source) && attemptedGeometryChange;
  ...
  if (attemptedGeometryChange) {
    logGeometryWrite({
      ...
      after: {
        ...
        x: sanitized.canvasPosition?.x,
        y: sanitized.canvasPosition?.y,
      },
      blocked,
    });
  }
...
export function useTaskOperations(
  ...
  const createTask = async (
    ...
    try {
      const instances: TaskInstance[] = taskData.instances
        ? [...taskData.instances]
        : [];
      if (
        instances.length === 0 &&
      ...
      if (taskData.parentTaskId) {
        const parentTask = _rawTasks.value.find(
      ...
      const newTask: Task = {
        ...
        dueDate: normalizeDueDate(
          taskData.dueDate || taskData.scheduledDate || "",
        ),
        projectId,
        createdAt: new Date(),
        updatedAt: new Date(),
        ...
        workspaceId:
          taskData.workspaceId !== undefined
      ...
      if (existingIdx !== -1) {
        console.warn(
          `[TASKS] createTask: duplicate push prevented for ${taskId.slice(0, 8)}, updating in-place`,
        );
      ...
      if (import.meta.env.DEV) {
        console.log(
          `[BUG-1329] Echo protection active for new task ${taskId.slice(0, 8)}`,
        );
      }
      ...
      try {
        ...
        if (!userId) {
          console.warn(
            "[SYNC] Skipping sync queue: user not authenticated; task requires guest storage",
          );
        ...
        console.warn(
          "[SYNC-QUEUE] Failed to queue create; task remains local-first and will be retried by cache/session recovery:",
          queueError,
        );
      }
      ...
      try {
        ...
        console.warn(
          "[READ-CACHE] Failed to persist newly created task:",
          cacheError,
        );
      }
      ...
      if (newTask.workspaceId) {
        import("@/composables/supabase/useWorkspaceActivity")
          .then(({ useWorkspaceActivity }) => {
      ...
      try {
        ...
        console.warn(
          "[READ-CACHE] Failed to remove rolled-back task from cache:",
          rollbackCacheError,
        );
      }
      throw error;
  ...
  const updateTask = async (
    ...
    try {
      ...
      const hasGeometryChange =
        ("parentId" in updates && updates.parentId !== task.parentId) ||
        ("canvasPosition" in updates &&
      ...
      if (import.meta.env.DEV) {
        if ("parentId" in updates && updates.parentId !== task.parentId) {
          console.log(
            `📍 [GEOMETRY-${source}] Task ${taskId.slice(0, 8)}... parentId: "${task.parentId ?? "none"}" → "${updates.parentId ?? "none"}"`,
            {
              taskTitle: task.title?.slice(0, 30),
              source,
        ...
        if (
          ...
          if (oldPos?.x !== newPos?.x || oldPos?.y !== newPos?.y) {
            console.log(
              `📍 [GEOMETRY-${source}] Task ${taskId.slice(0, 8)}... pos: (${oldPos?.x?.toFixed(0) ?? "?"},${oldPos?.y?.toFixed(0) ?? "?"}) → (${newPos?.x?.toFixed(0) ?? "null"},${newPos?.y?.toFixed(0) ?? "null"})`,
              {
                taskTitle: task.title?.slice(0, 30),
                source,
                stack: new Error().stack?.split("\n").slice(2, 5).join(" <- "),
        ...
        traceCanvasDone("updateTask:geometry-write", {
          ...
          updates: {
            ...
            canvasPosition: updates.canvasPosition
              ? {
                  x: Math.round(updates.canvasPosition.x),
                  y: Math.round(updates.canvasPosition.y),
                }
          ...
          before: {
            canvasPosition: task.canvasPosition
              ? {
                  x: Math.round(task.canvasPosition.x),
                  y: Math.round(task.canvasPosition.y),
                }
              : null,
      ...
      if (
        ...
        task.canvasPosition &&
        !updates.instances &&
        (!task.instances || !task.instances.length)
      ) {
      ...
      if ("projectId" in updates) {
        const isUncategorized =
          !updates.projectId ||
          updates.projectId === "1" ||
          updates.projectId === UNCATEGORIZED_PROJECT_ID;
      ...
      if ("title" in updates) {
        ...
        updates.title =
          safeTitle === FALLBACK_TASK_TITLE && isRealTaskTitle(task.title)
            ? task.title
            : safeTitle;
      }
      ...
      const newVersion = hasGeometryChange
        ? currentVersion + 1
        : currentVersion;
      ...
      if ("status" in updates) {
        ...
        if (wasNotDone && isNowDone) {
          ...
          console.log(
            `✅ [DONE-ZONE] Task "${task.title?.slice(0, 30)}" marked done, completedAt set, inbox cleared`,
          );
          ...
          try {
            ...
            if (
              ...
              console.log(
                `⏱️ [TIMER] Auto-stopped timer for completed task "${task.title?.slice(0, 30)}"`,
              );
            }
          } catch (e) {
          ...
          if (task.recurrenceRule) {
            try {
              ...
              let nextDueDate = computeNextDueDate(
                currentDueDate,
                task.recurrenceRule,
                count,
              );
              ...
              if (nextDueDate && nextDueDate <= today) {
                ...
                try {
                  ...
                  if (sbClient) {
                    ...
                    if (existingClones && existingClones.length > 0) {
                      console.log(
                        `[RECURRENCE] BUG-1531: Skipping clone — active clone already exists in DB for chain ${dedupChainId.slice(0, 8)}`,
                      );
                      skipClone = true;
                    }
                  }
                  ...
                  console.warn(
                    "[RECURRENCE] BUG-1531: DB dedup check failed, proceeding with clone:",
                    e,
                  );
                ...
                if (!skipClone) {
                  try {
                    ...
                    console.log(
                      `🔄 [RECURRENCE] Cloned recurring task "${task.title?.slice(0, 30)}" → next due: ${nextDueDate} (occurrence #${count})`,
                    );
                  } catch (cloneError: any) {
                    // DB unique constraint (idx_unique_recurrence_occurrence) catches cross-device race
                    if (
                      cloneError?.code === "23505" ||
                      cloneError?.message?.includes("duplicate key") ||
                      cloneError?.message?.includes("unique")
                    ) {
                      console.log(
                        `[RECURRENCE] Clone race resolved by DB constraint for chain ${(task.recurrenceParentId || task.id).slice(0, 8)}`,
                      );
                    } else {
                      throw cloneError;
                    }
                ...
                console.log(
                  `⏳ [RECURRENCE] Deferred clone for "${task.title?.slice(0, 30)}" → next due: ${nextDueDate} (future, will create on app load)`,
                );
              } else {
                console.log(
                  `🏁 [RECURRENCE] Recurring task "${task.title?.slice(0, 30)}" ended (${task.recurrenceRule.endType})`,
                );
              }
            } catch (e) {
        ...
        // Clear completedAt when status changes FROM 'done' (task reopened)
          ...
          console.log(
            `🔄 [DONE-ZONE] Task "${task.title?.slice(0, 30)}" reopened, completedAt cleared`,
          );
        }
      }
      ...
      if (
        ...
        try {
          ...
          const parentGroup = canvasStore.groups.find(
            (g) => g.id === task.parentId,
          );
          const parentIsSmart = parentGroup
            ? !!detectPowerKeyword(parentGroup.name)
            : false;
          const taskIsOnCanvas =
          ...
          if (taskIsOnCanvas && (parentIsSmart || !task.parentId)) {
            const newDueDate = updates.dueDate
              ...
              : undefined;
            const match = findMatchingGroupForDueDate(
              newDueDate,
              canvasStore.groups,
            );
      ...
      if (import.meta.env.DEV && syncedUpdates.status !== undefined) {
        console.log(
          `[BUG-1451] updateTask: ${taskId.slice(0, 8)} status: ${task.status} → ${syncedUpdates.status}`,
        );
      }
      ...
      if (
        task.canvasPosition &&
        syncedUpdates.canvasPosition === undefined &&
        "canvasPosition" in syncedUpdates
      ) {
      ...
      try {
        ...
        if (
          ...
          changedKeys.has("canvasPosition") ||
          changedKeys.has("parentId")
        ) {
        ...
        if (changedKeys.has("projectId")) {
          payload.project_id = isValidUUID(updatedTask.projectId)
        ...
        if (changedKeys.has("laneId")) {
          payload.lane_id = isValidUUID(updatedTask.laneId)
            ? updatedTask.laneId
            : null;
        }
        ...
        if (changedKeys.has("isCompletionRecord")) {
          payload.is_completion_record =
            updatedTask.isCompletionRecord ?? false;
        }
        // BUG-1302: Include instances in sync queue payload
        // Without this, calendar time blocks aren't backed up by the sync queue
        if (
          changedKeys.has("instances") &&
          updatedTask.instances !== undefined
        ) {
        ...
        if (
          changedKeys.has("recurringInstances") &&
          updatedTask.recurringInstances !== undefined
        ) {
          payload.recurring_instances = JSON.parse(
            JSON.stringify(updatedTask.recurringInstances),
          );
        }
        ...
        if (
          changedKeys.has("miniCanvasEdges") &&
          updatedTask.miniCanvasEdges !== undefined
        ) {
          payload.mini_canvas_edges = JSON.parse(
            JSON.stringify(updatedTask.miniCanvasEdges),
          );
        }
        ...
        if (
          changedKeys.has("recurrenceParentId") &&
          updatedTask.recurrenceParentId
        ) {
          payload.recurrence_parent_id = updatedTask.recurrenceParentId;
        }
        if (
          changedKeys.has("recurrenceCount") &&
          updatedTask.recurrenceCount !== undefined
        ) {
        ...
        if (
          changedKeys.has("reminders") &&
          updatedTask.reminders !== undefined
        ) {
          payload.reminders = JSON.parse(JSON.stringify(updatedTask.reminders));
        }
        if (
          changedKeys.has("attachments") &&
          updatedTask.attachments !== undefined
        ) {
          payload.attachments = JSON.parse(
            JSON.stringify(updatedTask.attachments),
          );
        }
        ...
        if (
          changedKeys.has("planningNotes") &&
          updatedTask.planningNotes !== undefined
        ) {
          payload.planning_notes = JSON.parse(
            JSON.stringify(updatedTask.planningNotes || []),
          );
        }
        ...
        if (changedKeys.has("dependsOn")) {
          const validDeps = (updatedTask.dependsOn || []).filter((id) =>
            isValidUUID(id),
          );
        ...
        if (changedKeys.has("parentTaskId")) {
          payload.parent_task_id = isValidUUID(updatedTask.parentTaskId)
        ...
        if (canonicalTaskPatch) {
          console.error(
            "[CANONICAL-SYNC] Failed to durably queue task patch; rolling back:",
            errorMsg,
          );
          const rollbackIndex = _rawTasks.value.findIndex(
          ...
          showToast(
            "This change was not safely queued. The task was restored.",
            "error",
          );
          throw queueError;
        }
        console.warn(
          "[SYNC-QUEUE] Failed to queue update, falling back to direct save:",
          errorMsg,
        );
        ...
        try {
          await saveSpecificTasks(
            [updatedTask],
            `updateTask-fallback-${taskId}`,
          );
          persisted = true;
        } catch (saveError) {
          persistenceError = saveError;
          console.warn(
            `[SYNC-QUEUE] Fallback save also failed for ${taskId}:`,
            saveError,
          );
        }
      }
      ...
      if (!persisted) {
        ...
        if (rollbackIndex !== -1) {
          console.error(
            `❌ [TASK] All persistence paths failed for ${taskId}, rolling back optimistic update`,
          );
          _rawTasks.value[rollbackIndex] = previousTask;
        }
        removePendingWrite(taskId);
  ...
  const deleteTask = async (
    ...
    if (index === -1) {
      console.warn(
        `⚠️ Task not found for deletion: ${taskId} (source: ${source})`,
      );
      return;
    }
    ...
    console.log(
      `🗑️ [DELETE] "${deletedTask.title?.slice(0, 30)}" (${taskId.slice(0, 8)}) — source: ${source}`,
    );
    manualOperationInProgress.value = true;
    ...
    if (import.meta.env.DEV) {
      console.log(
        `[BUG-1451] deleteTask: ${taskId.slice(0, 8)} "${deletedTask.title?.slice(0, 20)}" spliced from _rawTasks`,
      );
    }
    ...
    try {
      ...
      console.warn(
        `⚠️ [DELETE] Failed to queue delete for ${taskId.slice(0, 8)}, restoring the task:`,
        queueError,
      );
      if (!_rawTasks.value.some((task) => task.id === taskId)) {
        _rawTasks.value.splice(
          Math.min(index, _rawTasks.value.length),
          0,
          deletedTask,
        );
      }
    ...
    try {
      ...
      console.warn(
        `⚠️ [DELETE] Durable delete queued but read cache update failed for ${taskId.slice(0, 8)}:`,
        cacheError,
      );
    }
  ...
  const permanentlyDeleteTask = async (taskId: string) => {
    ...
    try {
      ...
      if (!hardDeleteCompleted && deletedTask.recurrenceRule) {
        ...
        const { showToast } = useToast();
        showToast(
          "Permanent delete was not confirmed. The recurring task has been restored.",
          "error",
        );
        throw error;
      }
      if (
        !hardDeleteCompleted &&
        shouldKeepPermanentDeleteLocallyOnRemoteFailure(error)
      ) {
        logPermanentDeleteTrace(
          taskId,
          "store.remote-delete-failed-keeping-local",
          {
            rawTaskCount: _rawTasks.value.length,
          },
        );
        ...
        try {
          ...
          logPermanentDeleteTrace(
            taskId,
            "store.remote-failed-permanent-delete-queued",
            {
              rawTaskCount: _rawTasks.value.length,
            },
          );
          try {
            ...
            console.warn(
              `⚠️ [PERMANENT-DELETE] Durable fallback queued but read cache update failed for ${taskId.slice(0, 8)}:`,
              cacheError,
            );
          }
          return;
          ...
          logPermanentDeleteTrace(
            taskId,
            "store.remote-failed-permanent-delete-queue-error",
            {
              error:
          ...
          console.warn(
            `⚠️ [PERMANENT-DELETE] Failed to queue fallback delete for ${taskId.slice(0, 8)}; restoring the task:`,
            queueError,
          );
          if (!_rawTasks.value.some((task) => task.id === taskId)) {
            _rawTasks.value.splice(
          ...
          const { showToast } = useToast();
          showToast(
            "Permanent delete was not saved. The task has been restored.",
            "error",
          );
          throw queueError;
        }
  ...
  const bulkPermanentlyDeleteTasks = async (taskIds: string[]) => {
    ...
    const selectedTasks = originalTasks.filter((task) =>
    ...
    try {
      ...
      const tasksAddedWhileDeleting = _rawTasks.value.filter(
      ...
      const { showToast } = useToast();
      showToast(
        "Permanent delete failed. Every selected task has been restored.",
        "error",
      );
      throw error;
    } finally {
  ...
  const clearRecurrenceChain = async (
    ...
    const chainId = task.recurrenceParentId || task.id;
    const chainMembers = _rawTasks.value.filter(
      (t) => t.id === chainId || t.recurrenceParentId === chainId,
    );
  ...
  const skipRecurringOccurrence = async (taskId: string) => {
    if (recurringSkipInFlight.has(taskId)) {
      console.warn(
        `[RECURRENCE-SKIP] Already in flight for ${taskId}, skipping duplicate request`,
      );
      return;
    }
    ...
    try {
      ...
      const currentDueDate =
        task.dueDate?.substring(0, 10) || formatDateKey(new Date());
  ...
  const stopRecurrence = async (taskId: string) => {
    if (recurringStopInFlight.has(taskId)) {
      console.warn(
        `[RECURRENCE-STOP] Already in flight for ${taskId}, skipping duplicate request`,
      );
      return;
    }
  ...
  const bulkDeleteTasks = async (taskIds: string[]) => {
    ...
    try {
      ...
      if (failed.length > 0) {
        ...
        const originalIds = new Set(originalTasks.map((task) => task.id));
        const tasksAddedWhileQueued = _rawTasks.value.filter(
          (task) => !originalIds.has(task.id),
        );
        _rawTasks.value = [
          ...originalTasks.filter(
            (task) => !taskIds.includes(task.id) || failedIds.has(task.id),
          ),
      ...
      try {
        ...
        console.warn(
          "[BULK-DELETE] Durable queue results could not be projected to the read cache:",
          cacheError,
        );
      }
      ...
      if (failed.length > 0) {
        const { showToast } = useToast();
        showToast(
          `${failed.length} task${failed.length === 1 ? "" : "s"} could not be deleted and were restored.`,
          "error",
        );
        throw new Error(
          `${failed.length} task delete${failed.length === 1 ? "" : "s"} could not be durably queued: ${failed
            .map(({ error }) =>
  ...
  const performDoneForNow = async (
    ...
    if (!authStore.user?.id) {
      ...
      const taskIndex = _rawTasks.value.findIndex(
        (candidate) => candidate.id === taskId,
      );
      ...
      try {
        ...
        await saveTasksToStorage(
          [..._rawTasks.value],
          "done-for-now-guest-durability",
        );
      } catch (error) {
        const completionIndex = _rawTasks.value.findIndex(
          (candidate) => candidate.id === completionRecord.id,
        );
        if (completionIndex !== -1) _rawTasks.value.splice(completionIndex, 1);
        const livingTaskIndex = _rawTasks.value.findIndex(
          (candidate) => candidate.id === taskId,
        );
        if (livingTaskIndex !== -1)
          _rawTasks.value.splice(livingTaskIndex, 1, taskBeforeCompletion);
        try {
          await cacheTasks([..._rawTasks.value], { throwOnError: true });
        } catch (rollbackError) {
          console.error(
            "[DONE-FOR-NOW] Failed to restore guest read cache after persistence failure:",
            rollbackError,
          );
        }
        throw error;
    ...
    if (typeof navigator !== "undefined" && !navigator.onLine) {
      ...
      if (!nextDueDate) {
        throw new Error(
          "Done for now could not compute the next occurrence while offline",
        );
      }
    ...
    if (!completed)
      throw new Error(
        "Done for now receipt did not include the completed occurrence",
      );
    ...
    console.log(
      `[DONE-FOR-NOW] "${task.title?.slice(0, 30)}" occurrence completed, next: ${next?.dueDate || "ended"}`,
    );
  };
  ...
  const doneForNow = async (
    ...
    if (existingOperation) {
      console.warn(
        `[DONE-FOR-NOW] Already in flight for ${taskId}, joining existing operation`,
      );
  ...
  const createTaskInstance = async (
    taskId: string,
    instanceData: Omit<TaskInstance, "id">,
  ) => {
    const task = _rawTasks.value.find((t) => t.id === taskId);
    if (!task) return null;
  ...
  const deleteTaskInstance = async (taskId: string, instanceId: string) => {
    ...
    if (task.recurringInstances && task.recurringInstances.length > 0) {
      const filtered = task.recurringInstances.filter(
    ...
    if (Object.keys(updates).length === 0) {
      console.warn(
        `[deleteTaskInstance] instanceId "${instanceId}" not found in task "${task.title?.slice(0, 30)}". Instances: ${task.instances?.length || 0}, RecurringInstances: ${task.recurringInstances?.length || 0}`,
      );
      return;
    }
    ...
    const remainingRecurring =
      updates.recurringInstances ?? task.recurringInstances ?? [];
    if (
      remainingInstances.length === 0 &&
      remainingRecurring.length === 0 &&
      !task.canvasPosition
    ) {
      updates.isInInbox = true;
    }
   ...
   */
  const updateTaskWithSchedule = async (
    taskId: string,
    schedule: {
  ...
  const moveTaskToSmartGroup = async (taskId: string, type: string) => {
    ...
    switch (type.toLowerCase()) {
      ...
      default:
        ...
        console.warn(
          `⚠️ [TASK-114] Unknown smart group type: "${type}" - no update performed`,
        );
        return;
    }
  ...
  const moveTaskToDate = async (taskId: string, dateColumn: string) => {
    ...
    switch (dateColumn) {
      ...
      case "thisWeek":
        target = new Date(today);
        target.setDate(today.getDate() + (7 - today.getDay()));
        break;
      case "nextWeek":
        target = new Date(today);
        target.setDate(today.getDate() + ((8 - today.getDay()) % 7 || 7));
        break;
      case "later":
        target = new Date(today);
  ...
  const moveTaskToPriority = async (
    taskId: string,
  ...
  const setActiveStatusFilter = (status: string | null) => {
    activeStatusFilter.value =
      status === "all" || status === null ? null : status;
  ...
  const setActiveDurationFilter = (
    duration: "quick" | "short" | "medium" | "long" | "unestimated" | null,
  ) => {
  ...
  const toggleDurationFilter = (
    ...
    setActiveDurationFilter(
      activeDurationFilter.value === duration ? null : duration,
    );
  };

  const getTask = (taskId: string) =>
    _rawTasks.value.find((t) => t.id === taskId);

  const getUncategorizedTaskCount = () => {
    const { isUncategorizedTask } = useSmartViews();
    return _rawTasks.value.filter(
      (t) =>
        t.status !== "done" &&
        !t._soft_deleted &&
        !t.isPinned &&
  ...
  const getNestedTasks = (parent: string | null = null) =>
    _rawTasks.value.filter((t) => t.parentTaskId === parent);
  const getTaskChildren = (taskId: string) =>
    _rawTasks.value.filter((t) => t.parentTaskId === taskId);
  ...
  const hasNestedTasks = (id: string) =>
    _rawTasks.value.some((t) => t.parentTaskId === id);
