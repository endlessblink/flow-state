...
import {
  useTimerSync,
  DEVICE_LEADER_TIMEOUT_MS,
} from "@/composables/timer/useTimerSync";
import { PENDING_WRITE_TIMEOUT_MS } from "@/config/timing";
import {
  subscribeLocalApiTimerMutations,
  syncLocalApiTimerSnapshot,
...
export const useTimerStore = defineStore("timer", () => {
  ...
  const { fetchActiveTimerSession, saveActiveTimerSession, claimLeadership } =
    useSupabaseDatabase();
  ...
  const settings = reactive({
    ...
    set workDuration(val) {
      settingsStore.updateSetting("workDuration", val);
    },
    get shortBreakDuration() {
      return settingsStore.shortBreakDuration;
    },
    set shortBreakDuration(val) {
      settingsStore.updateSetting("shortBreakDuration", val);
    },
    get longBreakDuration() {
      return settingsStore.longBreakDuration;
    },
    set longBreakDuration(val) {
      settingsStore.updateSetting("longBreakDuration", val);
    },
    get autoStartBreaks() {
      return settingsStore.autoStartBreaks;
    },
    set autoStartBreaks(val) {
      settingsStore.updateSetting("autoStartBreaks", val);
    },
    get autoStartPomodoros() {
      return settingsStore.autoStartPomodoros;
    },
    ...
    set playNotificationSounds(val) {
      settingsStore.updateSetting("playNotificationSounds", val);
    },
  });
  ...
  const notifications = useTimerNotifications({
    startTimer: (taskId: string, duration: number, isBreak: boolean) =>
      startTimer(taskId, duration, isBreak),
    addExtraTime: (seconds: number) => addExtraTime(seconds),
    getSettings: () => ({
      shortBreakDuration: settings.shortBreakDuration,
      workDuration: settings.workDuration,
    }),
    findTaskTitle: (taskId: string) =>
      taskStore._rawTasks.find((t) => t.id === taskId)?.title,
  });
  ...
  const isKdeWidgetActive = computed(() => {
    ...
    const lastSeen =
      typeof session.deviceLeaderLastSeen === "number"
  ...
  const loadTodaySessionsFromDB = async (): Promise<void> => {
    ...
    try {
      ...
      const sessions: PomodoroSession[] = todaySessions.map(
      ...
      if (import.meta.env.DEV) {
        console.log(
          `🍅 [TIMER] Loaded ${sessions.length} completed sessions from DB for today`,
        );
      }
  ...
  const unsubscribeAuth = authStore.$subscribe(
    (mutation, state) => {
      if (
        !hasLoadedTodaySessions.value &&
        state.user?.id &&
        settingsStore.aiLearningEnabled
      ) {
  ...
  const switchTimerTask = async (taskId: string) => {
    if (!currentSession.value) return;
    if (import.meta.env.DEV) {
      console.log("🍅 [TIMER] switchTimerTask:", {
        from: currentSession.value.taskId,
        to: taskId,
      });
    }
  ...
  const startTimer = async (
    ...
    try {
      ...
      if (!claimedLeadership) {
        ...
        console.warn(
          "🍅 [TIMER] Leadership claim failed but proceeding - user action takes precedence",
        );
      }
      isLeader.value = true;
    ...
    try {
      ...
      if (userId && currentSession.value) {
        ...
        const { toSupabaseTimerSession } =
          await import("@/utils/supabaseMappers");
        const payload = toSupabaseTimerSession(
          currentSession.value,
          userId,
          deviceId,
        );
  ...
  const stopTimer = async () => {
    ...
    if (currentSession.value) {
      ...
      if (import.meta.env.DEV) {
        console.log(
          "🍅 [TIMER] stopTimer: Saving stopped session to DB for cross-device sync",
          {
      ...
      setTimeout(
        () => completedSessionIds.delete(stoppedSession.id),
      ...
      if (authStore.canSyncRemotely) {
        try {
          ...
          if (import.meta.env.DEV) {
            console.log(
              "🍅 [TIMER] stopTimer: Session saved to DB successfully",
            );
          }
        } catch (saveError) {
          console.warn(
            "🍅 [TIMER] stopTimer: remote session save failed after local stop; keeping timer stopped",
            saveError,
          );
        }
      } else if (import.meta.env.DEV) {
        console.log(
          "🍅 [TIMER] stopTimer: reconnect/offline grace, skipping remote session save",
        );
      }
    ...
    try {
      ...
      try {
        const completedForDb: PomodoroSession = {
          ...completedSession,
          startTime:
            completedSession.startTime instanceof Date
        ...
        if (authStore.canSyncRemotely) {
          ...
          if (import.meta.env.DEV) {
            console.log(
              "🍅 [TIMER] completeSession: Saved completed state to DB",
              { sessionId: completedSession.id },
            );
          }
        } else if (import.meta.env.DEV) {
          console.log(
            "🍅 [TIMER] completeSession: reconnect/offline grace, skipping remote session save",
          );
        }
        ...
        try {
          ...
          console.warn(
            "[SYNC-QUEUE] Failed to queue timer complete:",
            queueError,
          );
        }
      } catch (e) {
        console.warn(
          "🍅 [TIMER] completeSession: Failed to save to DB (session may reappear on sync):",
          e,
        );
      }
      ...
      if (
        authStore.canSyncRemotely &&
        settingsStore.aiLearningEnabled &&
        !session.isBreak &&
        session.taskId &&
        session.taskId !== "general"
      ) {
        ...
        insertPomodoroHistory({
          ...
          console.warn("[Timer] Failed to write pomodoro history:", err),
        );
      }
      ...
      await notifications.showTimerNotification(
        session.id,
        wasBreak,
        lastTaskId,
        wasKdeWidgetActive,
  ...
  const addExtraTime = async (seconds: number) => {
    ...
    const lastSession =
      completedSessions.value[completedSessions.value.length - 1];
    if (!lastSession) {
      // Fallback: no session to extend, start fresh
      console.warn(
        "🍅 [TIMER] addExtraTime: No recent session to extend, falling back to startTimer",
      );
