...
const S = {
  ...
  micBtnRec: 'width: 32px; height: 32px; border-radius: 50%; border: none; background: var(--color-danger); color: var(--text-primary); display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0;',
  ...
  waveBar: 'width: 2px; height: 4px; background: var(--color-danger); border-radius: var(--radius-xs);',
}
  ...
  render: () => ({
    ...
    template: `
      <div style="${S.sidebar}">
        <div style="${S.section}">
          <div style="${S.row}">
            <input style="${S.input} border-color: var(--color-danger); box-shadow: 0 0 0 2px var(--danger-bg-subtle);" placeholder="Listening..." />
