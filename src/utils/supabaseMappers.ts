...
const sanitizeUUID = (value: string | null | undefined): string | null => {
  ...
  if (!isValidUUID(value)) {
    if (!warnedInvalidUUIDs.has(value)) {
      ...
      console.warn(
        `[SUPABASE-MAPPER] Invalid UUID detected: "${value}", converting to null`,
      );
    }
    return null;
  }
...
export interface SupabaseGroup {
  ...
  position_json?: {
    ...
    height: number;
    [key: string]: unknown;
  } | null; // Fixed: Use legacy database column name
  position_version?: number; // Optimistic locking for canvas position sync
  ...
  power_keyword_json?:
    | import("../composables/useTaskSmartGroups").PowerKeywordResult
    | null;
...
export function toSupabaseGroup(
  group: CanvasGroup,
  userId: string,
): SupabaseGroup | null {
  ...
  if (!isValidUUID(group.id)) {
    if (!warnedLegacyGroupIds.has(group.id)) {
      ...
      if (import.meta.env.DEV) {
        console.debug(
          `[SUPABASE-MAPPER] Group "${group.name}" has legacy ID: "${group.id}" - skipping Supabase sync (local only)`,
        );
      }
    }
    return null;
  ...
  if (group.parentGroupId && !sanitizedParentGroupId) {
    console.warn(
      `[SUPABASE-MAPPER] Group "${group.name}" had invalid parentGroupId: "${group.parentGroupId}", sanitized to null`,
    );
  }
  ...
  return {
    ...
    property_value:
      typeof group.propertyValue === "object"
        ? JSON.stringify(group.propertyValue)
        : group.propertyValue,
    ...
    ...((group as CanvasGroup & { workspaceId?: string | null }).workspaceId
      ? {
          workspace_id: (group as CanvasGroup & { workspaceId?: string | null })
            .workspaceId,
        }
      : {}),
...
export function toSupabaseProject(
  ...
  if (project.parentId && !sanitizedParentId) {
    console.warn(
      `[SUPABASE-MAPPER] Project "${sanitizedName}" had invalid parentId: "${project.parentId}", sanitized to null`,
    );
  }
  if (!project.name) {
    console.warn(
      `[SUPABASE-MAPPER] Project "${sanitizedId}" had null/empty name, defaulting to "Unnamed Project"`,
    );
  }
  ...
  return {
    ...
    is_deleted:
      (project as Project & { isDeleted?: boolean }).isDeleted || false,
    deleted_at: (project as Project & { deletedAt?: string | Date }).deletedAt
      ? new Date(
          (project as Project & { deletedAt?: string | Date }).deletedAt as
            | string
            | Date,
        ).toISOString()
      : null,
    created_at:
      project.createdAt instanceof Date
        ...
        : project.createdAt,
    // Workspace collaboration — only include when set (safe before migration)
    ...((project as Project & { workspaceId?: string | null }).workspaceId
      ? {
          workspace_id: (project as Project & { workspaceId?: string | null })
            .workspaceId,
        }
      : {}),
    updated_at: new Date().toISOString(),
...
export function toSupabaseLane(lane: Lane, userId: string): SupabaseLane {
  ...
  if (!lane.name) {
    console.warn(
      `[SUPABASE-MAPPER] Lane "${lane.id}" had null/empty name, defaulting to "Unnamed Lane"`,
    );
  }
  return {
    id: lane.id,
    ...
    deleted_at: (lane as Lane & { deletedAt?: string | Date }).deletedAt
      ? new Date(
          (lane as Lane & { deletedAt?: string | Date }).deletedAt as
            | string
            | Date,
        ).toISOString()
      : null,
    created_at:
      lane.createdAt instanceof Date
        ? lane.createdAt.toISOString()
        : lane.createdAt,
    // Workspace collaboration — only include when set (safe before migration)
    ...((lane as Lane & { workspaceId?: string | null }).workspaceId
      ? {
          workspace_id: (lane as Lane & { workspaceId?: string | null })
            .workspaceId,
        }
      : {}),
    updated_at: new Date().toISOString(),
...
export function toSupabaseTask(task: Task, userId: string): SupabaseTask {
  ...
  const sanitizedDependsOn = (task.dependsOn || []).filter((id) =>
    isValidUUID(id),
  );

  // SAFETY: Ensure status is valid per database constraint (tasks_status_check)
  const sanitizedStatus = VALID_TASK_STATUSES.includes(
  ...
  if (import.meta.env?.DEV && sanitizedStatus !== dbStatus) {
    console.debug(
      `[TASK-1418] toSupabaseTask status mapping: '${sanitizedStatus}' → '${dbStatus}' for task ${task.id?.slice(0, 8)}`,
    );
  }
  ...
  return {
    ...
    ...(task.laneId !== undefined
      ? { lane_id: sanitizeUUID(task.laneId) }
      : {}),
    ...
    ...(task.recurrenceRule
      ? { recurrence_rule: JSON.parse(JSON.stringify(task.recurrenceRule)) }
      : {}),
    ...(task.recurrenceParentId
      ? { recurrence_parent_id: sanitizeUUID(task.recurrenceParentId) }
      : {}),
    ...(task.recurrenceCount != null
      ? { recurrence_count: task.recurrenceCount }
      : {}),
  };
}
...
export function fromSupabaseTask(record: SupabaseTask): Task {
  return {
    ...
    recurrenceRule: record.recurrence_rule as
      | import("../types/tasks").SimpleRecurrenceRule
      | undefined,
    ...
    reminders:
      (record.reminders as import("../types/notifications").TaskReminder[]) ||
      [],
    attachments:
      (record.attachments as import("../types/tasks").TaskAttachment[]) || [],
...
export function toSupabaseUserSettings(
  ...
  return {
    ...
    sidebar_collapsed:
      (settings as AppSettings & { sidebarCollapsed?: boolean })
        .sidebarCollapsed || false,
    board_density: settings.boardDensity || "comfortable",
    kanban_settings:
      (settings as AppSettings & { kanbanSettings?: Record<string, unknown> })
        .kanbanSettings || {},
    canvas_viewport:
      (
...
export function fromSupabaseUserSettings(
  record: SupabaseUserSettings,
): AppSettings {
  ...
  return {
    ...
    workDuration:
      record.work_duration ?? (base.workDuration as number | undefined),
    shortBreakDuration:
      record.short_break_duration ??
      (base.shortBreakDuration as number | undefined),
    longBreakDuration:
      record.long_break_duration ??
      (base.longBreakDuration as number | undefined),
    autoStartBreaks:
      record.auto_start_breaks ?? (base.autoStartBreaks as boolean | undefined),
    autoStartPomodoros:
      record.auto_start_pomodoros ??
      (base.autoStartPomodoros as boolean | undefined),
    playNotificationSounds:
      record.play_notification_sounds ??
      (base.playNotificationSounds as boolean | undefined),
    theme: record.theme ?? (base.theme as string | undefined),
    language: record.language ?? (base.language as string | undefined),
    sidebarCollapsed:
      record.sidebar_collapsed ??
      (base.sidebarCollapsed as boolean | undefined),
    boardDensity:
      record.board_density ?? (base.boardDensity as string | undefined),
    kanbanSettings:
      record.kanban_settings ??
      (base.kanbanSettings as Record<string, unknown> | undefined),
    canvasViewport:
      record.canvas_viewport ??
      (base.canvasViewport as
    ...
    powerGroupOverrideMode:
      (base.powerGroupOverrideMode as string | undefined) ?? "only_empty",
    textDirection: (base.textDirection as string | undefined) ?? "auto",
    enableDayGroupSuggestions:
      (base.enableDayGroupSuggestions as boolean | undefined) ?? true,
    enableDayGroupPositionRotation:
      (base.enableDayGroupPositionRotation as boolean | undefined) ?? true,
  } as unknown as AppSettings;
}

// -- Notification Mappers --

export function toSupabaseNotification(
...
export function fromSupabaseNotification(
  record: SupabaseNotification,
...
export function toSupabaseTimerSession(
  session: PomodoroSession,
  userId: string,
  deviceId: string,
): SupabaseTimerSession {
  // SAFETY: Validate session ID - generate new UUID if invalid (prevents timestamp IDs from breaking DB)
  const validSessionId = isValidUUID(session.id)
    ? session.id
    : crypto.randomUUID();

  if (!isValidUUID(session.id)) {
    console.warn(
      `[SUPABASE-MAPPER] Timer session had invalid ID: "${session.id}", generated new UUID: ${validSessionId}`,
    );
  }
...
export function fromSupabaseTimerSession(
  record: SupabaseTimerSession,
): PomodoroSession & {
  ...
  if (!isValidUUID(record.id)) {
    console.warn(
      `[SUPABASE-MAPPER] Loaded timer session had invalid ID: "${record.id}", generated new UUID: ${validId}`,
    );
  }
  ...
  return {
    ...
    deviceLeaderLastSeen: record.device_leader_last_seen
      ? new Date(record.device_leader_last_seen).getTime()
      : undefined,
    canonicalRevision: record.canonical_revision,
  };
}
...
export function toSupabaseQuickSortSession(
  summary: SessionSummary,
  userId: string,
): SupabaseQuickSortSession {
  ...
  if (!isValidUUID(summary.id)) {
    console.warn(
      `[SUPABASE-MAPPER] QuickSort session had invalid ID: "${summary.id}", generated new UUID: ${validId}`,
    );
  }
...
export function fromSupabaseQuickSortSession(
  record: SupabaseQuickSortSession,
): SessionSummary {
  return {
    id: record.id,
...
export function toSupabaseWorkProfile(
  profile: Partial<WorkProfile>,
  userId: string,
): Partial<SupabaseWorkProfile> {
  return {
    user_id: userId,
    ...(profile.workDays !== undefined && { work_days: profile.workDays }),
    ...(profile.daysOff !== undefined && { days_off: profile.daysOff }),
    ...(profile.heavyMeetingDays !== undefined && {
      heavy_meeting_days: profile.heavyMeetingDays,
    }),
    ...(profile.maxTasksPerDay !== undefined && {
      max_tasks_per_day: profile.maxTasksPerDay,
    }),
    ...(profile.preferredWorkStyle !== undefined && {
      preferred_work_style: profile.preferredWorkStyle,
    }),
    ...(profile.topPriorityNote !== undefined && {
      top_priority_note: profile.topPriorityNote,
    }),
    ...(profile.personalContext !== undefined && {
      personal_context: profile.personalContext,
    }),
    ...(profile.avgWorkMinutesPerDay !== undefined && {
      avg_work_minutes_per_day: profile.avgWorkMinutesPerDay,
    }),
    ...(profile.avgTasksCompletedPerDay !== undefined && {
      avg_tasks_completed_per_day: profile.avgTasksCompletedPerDay,
    }),
    ...(profile.peakProductivityDays !== undefined && {
      peak_productivity_days: profile.peakProductivityDays,
    }),
    ...(profile.avgPlanAccuracy !== undefined && {
      avg_plan_accuracy: profile.avgPlanAccuracy,
    }),
    ...(profile.weeklyHistory !== undefined && {
      weekly_history: profile.weeklyHistory,
    }),
    ...(profile.interviewCompleted !== undefined && {
      interview_completed: profile.interviewCompleted,
    }),
    ...(profile.memoryGraph !== undefined && {
...
export function fromSupabaseWorkProfile(
  record: SupabaseWorkProfile,
): WorkProfile {
  return {
    id: record.id,
    ...
    preferredWorkStyle:
      (record.preferred_work_style as WorkProfile["preferredWorkStyle"]) ||
      "balanced",
    ...
    weeklyHistory: (record.weekly_history ||
      []) as WorkProfile["weeklyHistory"],
