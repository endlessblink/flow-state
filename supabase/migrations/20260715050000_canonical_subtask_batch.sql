...
BEGIN
  ...
  LOOP
    ...
    IF v_kind = 'create' THEN
      ...
      v_normalized_operation := pg_catalog.jsonb_strip_nulls(
        pg_catalog.jsonb_build_object(
          'kind', 'create',
          'clientId', v_client_id,
          'subtaskId', v_subtask_id,
          'parentTaskId', p_task_id,
          'title', pg_catalog.btrim(v_operation->>'title'),
          'description', v_operation->'description',
          'doneEnough', CASE
            WHEN pg_catalog.jsonb_typeof(v_operation->'doneEnough') = 'string'
              THEN pg_catalog.to_jsonb(pg_catalog.btrim(v_operation->>'doneEnough'))
            ELSE NULL
          END,
          'estimateMinutes', v_operation->'estimateMinutes',
          'completedPomodoros', COALESCE(v_operation->'completedPomodoros', '0'::jsonb),
          'canvasPosition', v_operation->'canvasPosition',
          'isCompleted', COALESCE(v_operation->'isCompleted', 'false'::jsonb),
          'order', v_operation->'order'
        )
      );
    ELSE
  ...
  FOR v_operation IN
    SELECT operation.value
    FROM pg_catalog.jsonb_array_elements(v_normalized_operations) WITH ORDINALITY
      AS operation(value, ordinality)
    ORDER BY operation.ordinality
  LOOP
    v_subtask_id := v_operation->>'subtaskId';
  ...
  v_result_subtasks := public.flowstate_h5_apply_subtask_operations(
    COALESCE(v_task.subtasks, '[]'::jsonb), v_normalized_operations
  );
  v_read_back := public.flowstate_h4_task_read_back(p_task_id)
    || pg_catalog.jsonb_build_object(
