...
CREATE TABLE IF NOT EXISTS public.canonical_legacy_work_block_ids (
  user_id uuid NOT NULL,
  ...
  created_at timestamptz NOT NULL DEFAULT pg_catalog.clock_timestamp(),
  PRIMARY KEY (user_id, work_block_id)
);

CREATE INDEX IF NOT EXISTS canonical_legacy_work_block_ids_task_idx
  ON public.canonical_legacy_work_block_ids (task_id)
  WHERE retired_at IS NOT NULL;
ALTER TABLE public.canonical_legacy_work_block_ids ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.canonical_legacy_work_block_ids FROM PUBLIC, anon, authenticated;

CREATE OR REPLACE FUNCTION public.flowstate_guard_retired_legacy_work_blocks_v1()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
BEGIN
  IF NEW.instances IS DISTINCT FROM OLD.instances
     AND pg_catalog.jsonb_typeof(NEW.instances) = 'array'
     AND EXISTS (
       SELECT 1
       FROM public.canonical_legacy_work_block_ids claim
       CROSS JOIN LATERAL pg_catalog.jsonb_array_elements(NEW.instances) item(value)
       WHERE claim.task_id = NEW.id::text
         AND claim.retired_at IS NOT NULL
         AND item.value->>'id' = claim.work_block_id
     ) THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'retired canonical work-block identity cannot be reused';
  END IF;
  RETURN NEW;
END;
$$;

REVOKE ALL ON FUNCTION public.flowstate_guard_retired_legacy_work_blocks_v1()
  FROM PUBLIC, anon, authenticated;
DROP TRIGGER IF EXISTS flowstate_guard_retired_legacy_work_blocks
  ON public.tasks;
CREATE TRIGGER flowstate_guard_retired_legacy_work_blocks
  BEFORE UPDATE OF instances ON public.tasks
  FOR EACH ROW
  EXECUTE FUNCTION public.flowstate_guard_retired_legacy_work_blocks_v1();
...
DECLARE
  ...
  v_current_work_block_revision bigint;
  v_before jsonb;
  v_after jsonb;
  v_instances jsonb;
  v_normalized jsonb;
...
BEGIN
  -- The timezone catalog is an expensive system view. Read it once per command
  -- instead of once per existing recurrence/work-block instance.
  SELECT pg_catalog.array_agg(zone.name) INTO v_valid_timezones
  ...
  ELSE
    ...
    IF pg_catalog.jsonb_typeof(p_command->'workBlockId') <> 'string'
       OR nullif(pg_catalog.btrim(v_work_block_id),'') IS NULL
       OR v_work_block_id IS DISTINCT FROM pg_catalog.btrim(v_work_block_id)
       OR pg_catalog.char_length(v_work_block_id) > 200 THEN
      RETURN pg_catalog.jsonb_build_object('ok',false,'result','rejected','error',pg_catalog.jsonb_build_object('code','invalid_work_block','message','The command requires an existing stable work-block ID'));
    END IF;
    IF v_work_block_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' THEN
      v_work_block_id := pg_catalog.lower(v_work_block_id);
    END IF;
    ...
    ELSIF v_action = 'resize' THEN
      IF EXISTS (SELECT 1 FROM pg_catalog.jsonb_object_keys(p_command) k(key) WHERE k.key NOT IN ('action','workBlockId','duration','timezone','finishBy')) THEN
      ...
      v_timezone := p_command->>'timezone';
    ELSE
      IF EXISTS (SELECT 1 FROM pg_catalog.jsonb_object_keys(p_command) k(key) WHERE k.key NOT IN ('action','workBlockId','timezone')) THEN
  ...
  END IF;
  IF v_action IN ('create','move','resize','remove') AND (
       v_timezone IS NULL OR NOT (v_timezone = ANY(v_valid_timezones))) THEN
  ...
  v_request_hash := pg_catalog.encode(extensions.digest(pg_catalog.convert_to(public.flowstate_canonical_json_text_v1(v_normalized),'UTF8'),'sha256'),'hex');
  ...
  IF FOUND THEN
    ...
    ELSIF NOT p_preview AND v_existing.state='committed' AND v_existing.canonical_result IS NOT NULL THEN
        RETURN pg_catalog.jsonb_build_object('ok',true,'status','replayed','result','committed','requestHash',v_request_hash,'receipt',v_existing.canonical_result || pg_catalog.jsonb_build_object('ok',true,'status','replayed','replayed',true));
    END IF;
  END IF;
  ...
  ELSE
    ...
    END IF;
    IF NOT (v_before ? 'canonicalRevision') THEN
      v_current_work_block_revision := 0;
    ELSIF pg_catalog.jsonb_typeof(v_before->'canonicalRevision') <> 'number'
       OR (v_before->>'canonicalRevision')::numeric <> pg_catalog.trunc((v_before->>'canonicalRevision')::numeric)
       OR (v_before->>'canonicalRevision')::bigint < 1 THEN
      RETURN pg_catalog.jsonb_build_object('ok',false,'result','conflict','error',pg_catalog.jsonb_build_object('code','invalid_existing_work_block','message','The existing work-block revision is invalid'));
    ELSE
      v_current_work_block_revision := (v_before->>'canonicalRevision')::bigint;
    END IF;
    IF v_current_work_block_revision IS DISTINCT FROM p_work_block_revision THEN
    ...
    END IF;
    IF v_before ? 'timezone' AND v_before->>'timezone' <> v_timezone AND v_action IN ('resize','remove') THEN
      RETURN pg_catalog.jsonb_build_object('ok',false,'result','conflict','error',pg_catalog.jsonb_build_object('code','invalid_timezone','message','timezone must match the existing work block'));
    END IF;
    v_before := v_before || pg_catalog.jsonb_build_object(
      'taskId',v_task.id,
      'timezone',COALESCE(v_before->>'timezone',v_timezone),
      'canonicalRevision',v_current_work_block_revision
    );
    IF COALESCE(v_before->>'scheduledDate','') !~ '^\d{4}-\d{2}-\d{2}$'
       ...
       OR NOT ((v_before->>'timezone') = ANY(v_valid_timezones)) THEN
      ...
      IF pg_catalog.jsonb_typeof(v_other_task.instances)='array' THEN
        ...
        LOOP
          ...
          IF v_other->>'id' IS DISTINCT FROM v_work_block_id
             ...
             AND v_other_timezone = ANY(v_valid_timezones) THEN
  ...
  v_expected_preview_digest := pg_catalog.encode(extensions.digest(pg_catalog.convert_to(public.flowstate_canonical_json_text_v1(pg_catalog.jsonb_build_object('requestHash',v_request_hash,'preview',v_preview_payload,'expiresAt',v_preview_expiry)),'UTF8'),'sha256'),'hex');
  ...
  IF p_preview THEN
    ...
    ELSIF FOUND AND v_issued_preview.consumed_at IS NULL AND v_issued_preview.expires_at>pg_catalog.clock_timestamp() THEN
      v_expected_preview_digest := pg_catalog.encode(extensions.digest(pg_catalog.convert_to(public.flowstate_canonical_json_text_v1(pg_catalog.jsonb_build_object('requestHash',v_request_hash,'preview',v_preview_payload,'expiresAt',v_issued_preview.expires_at)),'UTF8'),'sha256'),'hex');
  ...
  IF v_action='create' THEN
    ...
    VALUES(v_work_block_id::uuid,v_actor,v_task.id::text,p_operation_id)
    ...
    END IF;
  ELSIF v_work_block_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' THEN
    INSERT INTO public.canonical_work_block_ids(work_block_id,user_id,task_id,operation_id)
    VALUES(v_work_block_id::uuid,v_actor,v_task.id::text,p_operation_id)
    ON CONFLICT (work_block_id) DO NOTHING;
    IF EXISTS (
      SELECT 1 FROM public.canonical_work_block_ids claim
      WHERE claim.work_block_id=v_work_block_id::uuid AND claim.task_id IS DISTINCT FROM v_task.id::text
    ) THEN
    ...
    END IF;
  ELSE
    INSERT INTO public.canonical_legacy_work_block_ids(
      user_id,work_block_id,task_id,operation_id
    ) VALUES(v_actor,v_work_block_id,v_task.id::text,p_operation_id)
    ON CONFLICT (user_id,work_block_id) DO NOTHING;
    IF EXISTS (
      SELECT 1 FROM public.canonical_legacy_work_block_ids claim
      WHERE claim.user_id=v_actor AND claim.work_block_id=v_work_block_id
        AND (claim.task_id IS DISTINCT FROM v_task.id::text OR claim.retired_at IS NOT NULL)
    ) THEN
      RETURN pg_catalog.jsonb_build_object('ok',false,'result','conflict','error',pg_catalog.jsonb_build_object('code','work_block_id_conflict','message','This legacy work-block ID is retired or belongs to another task'));
    END IF;
  END IF;
  ...
  IF v_action='remove'
     AND v_work_block_id !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' THEN
    UPDATE public.canonical_legacy_work_block_ids claim
    SET retired_at=pg_catalog.clock_timestamp()
    WHERE claim.user_id=v_actor AND claim.work_block_id=v_work_block_id AND claim.task_id=v_task.id::text;
  END IF;
  ...
  v_read_back_hash := pg_catalog.encode(extensions.digest(pg_catalog.convert_to(public.flowstate_canonical_json_text_v1(v_read_back),'UTF8'),'sha256'),'hex');
  v_receipt := pg_catalog.jsonb_build_object(
    'ok',true,'contractVersion',p_contract_version,'operationId',p_operation_id,'source',p_source,
