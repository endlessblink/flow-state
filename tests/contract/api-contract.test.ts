...
function extractReturnObjectTopLevelKeys(src: string): string[] {
  ...
  if (returnStart === -1)
    throw new Error("return { not found in toSupabaseTask");
...
describe("TASK-1586: API Contract Tests — supabaseMappers.ts", () => {
  it("1. toSupabaseTask() return object uses only valid DB column names", () => {
    ...
    const columnLikeKeys = keys.filter(
      (k) => !jsKeywords.has(k) && /^[a-z]/.test(k),
    );

    const unknownColumns = columnLikeKeys.filter(
      (k) => !VALID_TASK_COLUMNS.has(k),
    );
    expect(
      unknownColumns,
  ...
  it("2. fromSupabaseTask() reads from valid DB column names", () => {
    ...
    const unknownRefs = uniqueRefs.filter(
      (col) => !VALID_TASK_COLUMNS.has(col),
    );
    expect(
      unknownRefs,
  ...
  it('3. Sync orchestrator DELETE case uses "is_deleted", not "_soft_deleted"', () => {
    ...
    expect(
      hasBuggyField,
      "DELETE case must not use _soft_deleted as a DB column key",
    ).toBe(false);
  });
  ...
  it('4. Sync orchestrator DELETE case sets "deleted_at" alongside "is_deleted"', () => {
    ...
    expect(
      updateCallMatch,
      ".update({...}) call should exist in delete block",
    ).toBeTruthy();
  ...
  it("5. All 5 entity type table names in sync orchestrator are valid", () => {
    ...
    const valueMatches = [...tableMapBlock.matchAll(/:\s*'([a-z_]+)'/g)].map(
      (m) => m[1],
    );
    ...
    const unknownTables = valueMatches.filter(
      (t) => !validTableNames.includes(t),
    );
    expect(
      unknownTables,
      `Unknown table names as values: ${unknownTables.join(", ")}`,
    ).toEqual([]);
  });
  ...
  it("6. Task mapper preserves JSONB subtasks field without corruption", () => {
    ...
    expect(dbRecord.subtasks![0]).toMatchObject({
      id: "sub-1",
      title: "First subtask",
      done: false,
    });
  ...
  it("7. Task mapper preserves tags array without corruption", () => {
    ...
    expect(dbRecord.tags!.every((t: string) => typeof t === "string")).toBe(
      true,
    );
