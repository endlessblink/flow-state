    ...
    for (const file of frontendFiles) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const hasUserParam =
          ...
          /accept_workspace_invite|claim_timer_leadership|flowstate_done_for_now|flowstate_patch_task_v1|flowstate_has_task_tombstone|search_task_audit|transfer_workspace_ownership/m.test(ctx)
