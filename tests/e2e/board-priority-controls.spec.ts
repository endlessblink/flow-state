...
test.describe('Board priority and recurring filters', () => {
  ...
  test('shows high-first sorting and filters priority and recurring tasks', async ({ page }) => {
    ...
    const priorityFilter = page.locator('.filter-control[title="Priority"]')
    const recurringFilter = page.locator('.filter-control[title="Recurring tasks"]')
    await expect(priorityFilter).toBeVisible()
    await priorityFilter.locator('.select-trigger').evaluate((element: HTMLElement) => element.click())
    ...
    await recurringFilter.locator('.select-trigger').evaluate((element: HTMLElement) => element.click())
