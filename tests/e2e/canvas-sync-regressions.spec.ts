...
import {
  test,
  expect,
  readCanvasNodePositions,
  expectNoNodesMoved,
  waitForCanvasNodes,
} from "../fixtures/two-client";
...
const ROOT_TASKS = [
  {
    ...
    title: "Sync Regr Root 1",
    x: 2000,
    y: 2000,
  },
  {
    id: "d1000000-0000-4000-8000-000000000002",
    title: "Sync Regr Root 2",
    x: 2400,
    y: 2000,
  },
  {
    id: "d1000000-0000-4000-8000-000000000003",
    title: "Sync Regr Root 3",
    x: 2200,
    y: 2400,
  },
];
...
async function createMobileTasksClient(sourcePage: Page) {
  ...
  if (!browser)
    throw new Error("Mobile consistency drill requires a browser context");
...
test.describe("Recurring canvas/sync regressions (TASK-1871)", () => {
  ...
  test.skip(
    !SERVICE_ROLE_KEY,
    "requires SUPABASE_SERVICE_ROLE_KEY (set by scripts/run-e2e.sh)",
  );
  ...
  test.beforeEach(async () => {
    ...
    await admin
      .from("tombstones")
      .delete()
      .eq("entity_id", OFFLINE_RECURRING_TASK.id);
    ...
    await admin
      ...
      .delete()
      .eq("user_id", userId)
      .eq("title", CRASH_RECOVERY_QUICK_CREATE_TITLE);
    await admin.from("groups").delete().eq("id", GROUP_ID);
    await admin
      .from("tombstones")
      .delete()
      .in("entity_id", [...ALL_IDS, GROUP_ID]);
    ...
    for (const t of ROOT_TASKS) {
      const { error } = await admin.from("tasks").insert({
        ...
        priority: "medium",
        is_in_inbox: false,
        position: { x: t.x, y: t.y, format: "absolute" },
        position_version: 1,
      });
    ...
    expect(
      inboxError,
      `Failed to seed inbox task: ${inboxError?.message}`,
    ).toBeNull();
    ...
    expect(
      calendarError,
      `Failed to seed calendar task: ${calendarError?.message}`,
    ).toBeNull();
    ...
    expect(
      mobileError,
      `Failed to seed mobile task: ${mobileError?.message}`,
    ).toBeNull();
    ...
    expect(
      groupError,
      `Failed to seed regression group: ${groupError?.message}`,
    ).toBeNull();
    ...
    expect(
      childError,
      `Failed to seed child task: ${childError?.message}`,
    ).toBeNull();
    ...
    expect(
      recurringError,
      `Failed to seed recurring task: ${recurringError?.message}`,
    ).toBeNull();
  ...
  test.afterAll(async () => {
    ...
    await admin
      ...
      .delete()
      .eq("user_id", userId)
      .eq("name", "Monday"); // R7 migrated group
    await admin
      .from("tombstones")
      .delete()
      .in("entity_id", [...ALL_IDS, GROUP_ID]);
  ...
  test("R7 - legacy day-group migrates to a UUID and syncs across clients", async ({
    clientA,
    clientB,
  }) => {
    ...
    await expect(async () => {
      const { data } = await admin
        ...
        .eq("name", "Monday");
      expect(
        (data ?? []).some((g: any) => UUID_RE.test(g.id)),
        "no UUID Monday group in DB",
      ).toBe(true);
    ...
    await expect(async () => {
      ...
      expect(hasMonday, "client B never received the migrated group").toBe(
        true,
      );
    }).toPass({ timeout: 12_000 });
  ...
  test("R1 - adding a task to the canvas does not shift existing nodes", async ({
    clientA,
  }) => {
    await gotoCanvasReady(clientA);
    ...
    const writes = await clientA.evaluate(
      () => (window as any).__FlowStateGeometryWrites ?? [],
    );
    const movedOthers = (writes as any[]).filter(
      (w) => w.entityId !== INBOX_TASK.id,
    );
    expect(
      movedOthers,
      `Other entities got geometry writes on drop: ${JSON.stringify(movedOthers)}`,
    ).toEqual([]);
  ...
  test("R2 - task field update on client A reaches independent client B via Realtime", async ({
    clientA,
    clientB,
  }) => {
    ...
    clientB.on("console", (message) => {
      ...
      if (
        ...
        text.includes("[CANONICAL-CATCHUP]")
      ) {
  ...
  test("R9 - a created task exists in Supabase, reaches another client, and survives reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoCanvasReady(clientA);
  ...
  test("R10 - a task created offline drains after reconnect, reaches another client, and survives reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoCanvasReady(clientA);
  ...
  test("R16 - workspace switching never hides, leaks, or loses newly created tasks", async ({
    ...
    clientA.on("console", (message) => {
      const text = message.text();
      if (
        /SYNC|WORKSPACE|Task could not be saved|Failed to queue/i.test(text)
      ) {
    ...
    try {
      ...
      await expect(
        clientA.getByText(personalTitle, { exact: true }),
      ).toBeVisible();
      ...
      await quickCreate.press("Enter");
      await expect(
        clientA.getByText(workspaceTitle, { exact: true }),
      ).toBeVisible();
      ...
      try {
        ...
        const syncDiagnostics = await clientA.evaluate(async () => {
          ...
          return {
            ...
            failedOperations: (await getFailedOperations()).map(
              (operation) => ({
        ...
        console.log(
          `R16 sync diagnostics:\n${JSON.stringify(syncDiagnostics, null, 2)}\n${clientLogs.join("\n")}`,
        );
        throw error;
      }

      const backupScopeProof = await clientA.evaluate(
        async ({ personalTitle, workspaceTitle }) => {
          const { default: useBackupSystem } =
            await import("/src/composables/useBackupSystem.ts");
          ...
          return {
            titles: backup.tasks
              .filter(
                (task) =>
                  task.title === personalTitle || task.title === workspaceTitle,
              )
              .map((task) => task.title),
      ...
      expect(backupScopeProof.titles).toEqual(
        expect.arrayContaining([personalTitle, workspaceTitle]),
      );
      ...
      await expect(
        clientB.getByText(personalTitle, { exact: true }),
      ).toBeVisible();
      ...
      await expect(
        clientB.getByText(workspaceTitle, { exact: true }),
      ).toBeVisible();
      await expect(
        clientB.getByText(personalTitle, { exact: true }),
      ).toHaveCount(0);
  ...
  test("R13 - Quick Create works offline, drains after reconnect, reaches another client, and survives reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoCanvasReady(clientA);
      ...
      .poll(
        async () =>
          clientA.evaluate((title) => {
            ...
            return (
              tasks.rawTasks.find((candidate: any) => candidate.title === title)
                ?.id ?? null
            );
  ...
  test("R23 - a stale Quick Create write survives a renderer crash and resumes after reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoCanvasReady(clientA);
    ...
    const crashedTaskId = await clientA.evaluate(async (title) => {
      ...
      const task = tasks.rawTasks.find(
        (candidate: any) => candidate.title === title,
      );
      if (!task)
        throw new Error("Quick Create task missing before crash simulation");
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        request.onsuccess = () => {
          const operation = request.result.find(
            (candidate) => candidate.entityId === task.id,
          );
          if (!operation) {
            reject(
              new Error(
                "Durable Quick Create operation missing before crash simulation",
              ),
            );
            return;
          }
      ...
      .poll(
        async () =>
          clientB.evaluate((id) => {
            ...
            return (
              tasks.rawTasks.find((candidate: any) => candidate.id === id)
                ?.title ?? null
            );
          }, crashedTaskId),
        { timeout: 20_000 },
      )
      .toBe(CRASH_RECOVERY_QUICK_CREATE_TITLE);
    await restartedClient.close();
  });

  test("R14 - Catalog right-click completion and reopen converge offline, cross-client, and after reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoCatalogReady(clientA);
    ...
    const taskRow = clientA
      .locator(".hierarchical-task-row")
    ...
    await clientA.evaluate(() => {
      const current = JSON.parse(
        localStorage.getItem("flowstate-filters") || "{}",
      );
    ...
    const completedTaskRow = clientA
      .locator(".hierarchical-task-row")
      .filter({ hasText: task.title })
      .first();
  ...
  test("R26 - Calendar right-click completion and reopen converge offline, cross-client, and after reload", async ({
    clientA,
    clientB,
  }) => {
        ...
        clientA.evaluate((taskId) => {
          ...
          return task
            ? {
                ...
                inCalendarFilteredTasks: tasks.calendarFilteredTasks.some(
                  (candidate: any) => candidate.id === taskId,
                ),
                visibleSlotDate:
                  document
                    .querySelector(".time-slot")
                    ?.getAttribute("data-slot-date") ?? null,
                scheduleMatchesVisibleDate:
                  task.instances?.[0]?.scheduledDate ===
                  document
                    .querySelector(".time-slot")
                    ?.getAttribute("data-slot-date"),
              }
            : null;
        }, CALENDAR_TASK.id),
      )
    ...
    const calendarEvent = clientA.locator(
      `.slot-task[data-task-id="${CALENDAR_TASK.id}"]`,
    );
    const clientBCalendarEvent = clientB.locator(
      `.slot-task[data-task-id="${CALENDAR_TASK.id}"]`,
    );
    ...
    await expect(
      clientA
        .locator(".slot-task")
        .filter({ hasText: CALENDAR_TASK.title })
        .first(),
    ).toBeVisible();
    await expect
      .poll(async () =>
  ...
  test("R29 - Calendar removal rejection stays visible and leaves the task intact", async ({
    clientA,
  }) => {
    await gotoCalendarReady(clientA);
    ...
    await expect(
      clientA.getByText(
        "Task could not be removed from Calendar. No changes were saved.",
        { exact: true },
      ),
    ).toBeVisible();
    await expect(calendarEvent).toBeVisible();
    await expect
      .poll(async () =>
        clientA.evaluate((taskId) => {
          ...
          return tasks.rawTasks.some(
            (candidate: any) => candidate.id === taskId && !candidate.isDeleted,
          );
        }, CALENDAR_TASK.id),
      )
      .toBe(true);
  ...
  test("R30 - Calendar removal works offline, preserves the task, and converges across views", async ({
    clientA,
    clientB,
  }) => {
    await gotoCalendarReady(clientA);
  ...
  test("R28 - Calendar next-occurrence completion works offline exactly once and survives reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoCalendarReady(clientA);
    ...
    const recurringEvent = clientA.locator(
      `.slot-task[data-task-id="${CALENDAR_RECURRING_TASK.id}"]`,
    );
    const clientBRecurringEvent = clientB.locator(
      `.slot-task[data-task-id="${CALENDAR_RECURRING_TASK.id}"]`,
    );
    ...
    await expect(async () => {
      const { data: livingTask, error: livingError } = await admin
        .from("tasks")
        .select(
          "id,due_date,status,recurrence_count,is_completion_record,is_deleted",
        )
        .eq("id", CALENDAR_RECURRING_TASK.id)
        .single();
      expect(livingError).toBeNull();
      expect(livingTask).toEqual(
        expect.objectContaining({
          id: CALENDAR_RECURRING_TASK.id,
          status: "planned",
          recurrence_count: 1,
      ...
      expect(String(livingTask?.due_date).slice(0, 10)).not.toBe(
        initialDueDate,
      );
  ...
  test("R27 - Mobile checkbox completion and reopen converge offline, cross-client, and after reload", async ({
    clientB,
  }) => {
    await gotoCanvasReady(clientB);
    const { context: mobileContext, page: mobileClient } =
      await createMobileTasksClient(clientB);
    try {
      const mobileTask = mobileClient
        .locator(".swipeable-task-item")
        .filter({ hasText: MOBILE_TASK.title })
        .first();
  ...
  test("R25 - Canvas right-click Mark as Done persists, converges, and survives reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoCanvasReady(clientA);
  ...
  test("R15 - Canvas next-occurrence completion works offline, commits once, converges, and survives reload", async ({
    clientA,
    clientB,
  }) => {
    ...
    clientA.on("console", (message) => {
      if (
        message.type() === "error" ||
        message.text().includes("[DONE-FOR-NOW]")
      ) {
  ...
  test("R17 - Board right-click completion works offline, converges, and survives reload", async ({
    clientA,
    clientB,
  }) => {
  ...
  test("R18 - Board recurring next-occurrence completion works offline exactly once", async ({
    clientA,
    clientB,
  }) => {
    await gotoBoardReady(clientA);
  ...
  test("R31 - recurring skip rejection stays visible and preserves the series", async ({
    clientA,
  }) => {
    await gotoBoardReady(clientA);
    ...
    await clientA.route(writePattern, (route) => {
      ...
      return route.fulfill({
        ...
        body: JSON.stringify({
          message: "Injected direct persistence rejection",
        }),
      });
    });
    await clientA.evaluate(() => {
      ...
      IDBObjectStore.prototype.add = function () {
        throw new DOMException(
          "Injected durable queue rejection",
          "UnknownError",
        );
      };
    });
    ...
    try {
      ...
      await expect(
        clientA.getByText(
          "Recurring task could not be changed. No changes were saved.",
          { exact: true },
        ),
      ).toBeVisible();
      await expect(recurringCard).toBeVisible();
  ...
  test("R32 - recurring skip works offline and produces exactly one next occurrence", async ({
    clientA,
    clientB,
  }) => {
    await gotoBoardReady(clientA);
  ...
  test("R33 - recurring stop rejection preserves the living series and reports failure", async ({
    clientA,
  }) => {
    await gotoBoardReady(clientA);
    ...
    await clientA.evaluate(() => {
      const originalAdd = IDBObjectStore.prototype.add;
      (window as any).__restoreRecurringStopQueue = () => {
        IDBObjectStore.prototype.add = originalAdd;
      };
      IDBObjectStore.prototype.add = function () {
        throw new DOMException(
    ...
    try {
      ...
      await clientA
        .getByText("Stop all future occurrences", { exact: true })
        .click();
  ...
  test("R34 - recurring stop works offline, converges, and cannot resurrect after reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoBoardReady(clientA);
    ...
    await expect(async () => {
      const { data, error } = await admin
        .from("tasks")
        .select("id,recurrence_rule,is_deleted")
        .eq("id", OFFLINE_RECURRING_TASK.id)
        .single();
      expect(error).toBeNull();
      expect(data).toEqual(
  ...
  test("R19 - Board right-click edit works offline, converges, and survives reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoBoardReady(clientA);
  ...
  test("R24 - two open windows replay one shared offline Board edit exactly once", async ({
    clientA,
    clientB,
  }) => {
    await gotoBoardReady(clientA);
    ...
    await expect
      .poll(async () =>
        competingWindow.evaluate((taskId) => {
          const root = document.querySelector("#app") as any;
          const tasks =
            root.__vue_app__._context.config.globalProperties.$pinia._s.get(
              "tasks",
  ...
  test("R20 - Board right-click delete works offline and cannot resurrect", async ({
    clientA,
    clientB,
  }) => {
    await gotoBoardReady(clientA);
      ...
      .poll(
        async () =>
          clientB.evaluate((taskId) => {
            ...
            return tasks.rawTasks.some(
              (candidate: any) =>
                candidate.id === taskId && !candidate.isDeleted,
            );
          }, task.id),
        { timeout: 20_000 },
      )
  ...
  test("R21 - Board delete rejection stays visible and leaves the task intact", async ({
    clientA,
  }) => {
    await gotoBoardReady(clientA);
    ...
    await expect(
      clientA.getByText("Task could not be deleted. No changes were saved.", {
        exact: true,
      }),
    ).toBeVisible();
    await expect(taskCard).toBeVisible();
  ...
  test("R22 - a rejected Board edit remains recoverable and converges after manual retry", async ({
    clientA,
    clientB,
  }) => {
    await gotoBoardReady(clientA);
    ...
    await expect(clientA.getByText("Corrupted", { exact: true })).toHaveCount(
      0,
    );
    await expect(
  ...
  test("R11 - offline edit and completion drain after reconnect and survive reload", async ({
    clientA,
    clientB,
  }) => {
    await gotoCanvasReady(clientA);
  ...
  test("R5 - moving a task on client A live-updates its position on client B", async ({
    clientA,
    clientB,
  }) => {
    await gotoCanvasReady(clientA);
  ...
  test("R8 - client B replays latest remote geometry after its interaction guard clears", async ({
    clientA,
    clientB,
  }) => {
    await gotoCanvasReady(clientA);
    ...
    const lockAccepted = await clientB.evaluate(async () => {
      const { useCanvasOperationState } =
        await import("/src/composables/canvas/useCanvasOperationState.ts");
      return useCanvasOperationState().startDrag([
        "deterministic-remote-projection-lock",
      ]);
    });
    ...
    await expect(async () => {
      const storePos = await clientB.evaluate((taskId) => {
        ...
        return (
          tasks.rawTasks.find((task: any) => task.id === taskId)
            ?.canvasPosition ?? null
        );
      }, ROOT_TASKS[0].id);
    ...
    const blockedNode = (await readCanvasNodePositions(clientB))[
      ROOT_TASKS[0].id
    ];
  ...
  test("R6 - moving a group on client A live-updates its position on client B", async ({
    clientA,
    clientB,
  }) => {
    const bLogs: string[] = [];
    ...
    await clientA.evaluate(
      ({ pos, gid }) => {
        ...
        return canvas.updateGroup(gid, {
          position: { x: pos.x, y: pos.y, width: 800, height: 600 },
        });
      },
    ...
    await expect(async () => {
      ...
      expect(
        dbx,
        `DB group x=${dbx} (want ${target.x}) — A never persisted the move.\nA SYNC logs:\n${aLogs.join("\n") || "(no sync logs)"}`,
      ).toBe(target.x);
    }).toPass({ timeout: 12_000 });
  ...
  test("R3 - opening the canvas performs no automated geometry writes", async ({
    clientA,
  }) => {
    ...
    const autoWrites = await clientA.evaluate(() =>
      ((window as any).__FlowStateGeometryWrites ?? []).map((w: any) => ({
        source: w.source,
        id: w.entityId,
      })),
    );
    expect(
      autoWrites,
      `Opening the canvas wrote geometry (auto-reposition): ${JSON.stringify(autoWrites)}`,
    ).toEqual([]);
  });

  // ── R4: deleting a group must not make its child tasks vanish ───────────────
  test("R4 - deleting a group keeps its child tasks on the canvas", async ({
    clientA,
  }) => {
    await gotoCanvasReady(clientA);
    await expect(clientA.locator(`[data-id="${CHILD_TASK.id}"]`)).toHaveCount(
      1,
      { timeout: 20_000 },
    );
    ...
    await expect(
      clientA.locator(`[data-id="section-${GROUP_ID}"]`),
    ).toHaveCount(0);
