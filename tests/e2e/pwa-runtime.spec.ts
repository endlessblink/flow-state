...
test.describe("SW Registration & Cache", () => {
  test("1 - SW installs and controls page", async ({ page }) => {
    ...
    test.skip(
      ...
      "environment-gated: service worker requires a production build (set TEST_URL)",
    );
    await page.goto("/");
  ...
  test("2 - Cache contains index.html and critical assets", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
    ...
    const cacheInfo = await page.evaluate(async () => {
      ...
      return {
        ...
        hasIndex: allUrls.some(
          (u) => u.endsWith("/") || u.includes("index.html"),
        ),
      };
    });
  ...
  test("3 - index.html served with no-cache or short cache header", async ({
    page,
  }) => {
  ...
  test("4 - JS chunks carry immutable or long cache headers", async ({
    page,
  }) => {
  ...
  test("7 - Manifest is valid and meets installability criteria", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("8 - No stale auth after simulated token expiry", async ({ page }) => {
    ...
    const fatalErrors = critical.filter(
      (e) =>
...
test.describe("Offline & Background Sync", () => {
  test("9 - Offline: app shell still renders", async ({ page, context }) => {
    ...
    test.skip(
      !process.env.TEST_URL,
      "environment-gated: offline app shell requires a production build (set TEST_URL)",
    );
    ...
    const isOfflinePage =
      bodyText.includes("offline") || bodyText.includes("ERR_INTERNET");
  ...
  test("12 - No chunk load errors during normal navigation", async ({
    page,
  }) => {
    const errors = collectConsoleErrors(page);
  ...
  test('15 - No "undefined" or "null" text rendered in any view', async ({
    page,
  }) => {
...
test.describe("Production Bug Detection", () => {
  test("16 - No chunk load failure errors during full app navigation", async ({
    page,
  }) => {
    const errors = collectConsoleErrors(page);
    ...
    expect(
      chunkErrors,
      `Chunk load errors: ${chunkErrors.join("\n")}`,
    ).toHaveLength(0);
  });
  ...
  test("18 - No mixed content (http:// from https:// page)", async ({
    page,
  }) => {
    ...
    expect(
      mixedRequests,
      `Mixed content requests: ${mixedRequests.join("\n")}`,
    ).toHaveLength(0);
  });
  ...
  test('20 - Build version sentinel defined (not "undefined")', async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
  ...
  test("21 - No white flash on initial load", async ({ page }) => {
    ...
    const isWhite =
      bgAfter === "rgb(255, 255, 255)" || bgAfter === "rgba(0, 0, 0, 0)";
    ...
    expect(isWhite, `Body background is white/transparent: ${bgAfter}`).toBe(
      false,
    );
  });
  ...
  test("24 - Cache versioning: only current version cache exists", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
    ...
    const precacheCaches = cacheNames.filter(
      (n) =>
        n.includes("workbox") ||
        n.includes("precache") ||
        n.includes("runtime"),
    ...
    const prefixes = new Set(
      precacheCaches.map((n) => n.replace(/-[\da-f]+$/, "")),
    );
  ...
  test("25 - All lazy-loaded routes resolve without chunk errors", async ({
    page,
  }) => {
    const errors = collectConsoleErrors(page);
    ...
    const chunkErrors = errors.filter(
      (e) => e.includes("chunk") || e.includes("dynamically imported module"),
    );

    expect(
      chunkErrors,
      `Chunk errors on routes: ${chunkErrors.join("\n")}`,
    ).toHaveLength(0);
    expect(
      failedRoutes,
      `Routes that rendered blank: ${failedRoutes.join(", ")}`,
    ).toHaveLength(0);
  });
});
