...
describe('AI memory schema contract', () => {
  ...
  it('keeps synthetic and workflow memory keyed by text, not UUID-only project ids', () => {
    ...
    expect(createTableBlock('ai_context_entities')).toContain('canonical_project_id text')
    expect(createTableBlock('ai_context_entities')).toContain('canonical_task_id text')
    expect(createTableBlock('ai_context_entities')).not.toContain('canonical_project_id text references public.projects(id)')
    expect(createTableBlock('ai_context_entities')).not.toContain('canonical_task_id text references public.tasks(id)')
    ...
    expect(createTableBlock('ai_recommendation_feedback')).toContain('task_id text')
    expect(createTableBlock('ai_recommendation_feedback')).not.toContain('task_id text references public.tasks(id)')
  ...
  it('keeps the live Supabase schema readiness checker aligned with server memory tables', () => {
    ...
    expect(liveSchemaChecker).toContain('AI_MEMORY_SCHEMA_RETRIES')
    expect(liveSchemaChecker).toContain('AI_MEMORY_SCHEMA_RETRY_MS')
    expect(liveSchemaChecker).toContain('PGRST205')
    expect(liveSchemaChecker).toContain('Schema cache still missing')
  ...
  it('keeps the live AI memory migration applier dry-run by default and double-confirmed', () => {
    ...
    expect(liveMigrationApplier).toContain('Running read-only VPS database preflight')
    expect(liveMigrationApplier).toContain("select current_database();")
    expect(liveMigrationApplier).toContain('REMOTE_DB_CONTAINER')
    expect(liveMigrationApplier).toContain('npm run check:ai-memory-schema')
    expect(liveMigrationApplier).toContain('AI_MEMORY_SCHEMA_RETRIES="${AI_MEMORY_SCHEMA_RETRIES:-12}"')
