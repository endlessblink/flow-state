...
describe('AI sidebar-first desktop experience', () => {
  ...
  it('documents weekly planning as a structured artifact instead of prose plus detached cards', () => {
    ...
    expect(broadClarification).toContain('broad task answer would otherwise rank')
    ...
    expect(src('src/composables/useAIChat.ts')).toContain('fetchAIRecommendationFeedback({ taskIds, entityKeys, limit: 20 })')
    expect(src('src/composables/useAIChat.ts')).toContain('...taskIdStrings.map(taskEntityKey)')
    expect(src('src/composables/useAIChat.ts')).toContain('...projectIdStrings.map(projectEntityKey)')
    expect(src('src/composables/useAIChat.ts')).toContain('recommendation feedback for ${target}')
    expect(src('src/composables/useAIChat.ts')).toContain('function feedbackTargetLabel(feedback: AIRecommendationFeedback)')
    expect(src('src/composables/useAIChat.ts')).toContain('function fallbackFeedbackSignal')
