...
describe('AI sidebar-first desktop experience', () => {
  ...
  it('documents weekly planning as a structured artifact instead of prose plus detached cards', () => {
    ...
    expect(src('src/components/settings/tabs/AISettingsTab.vue')).toContain('Server schema unavailable; chat is using local fallback and queued writes')
    expect(src('src/components/settings/tabs/AISettingsTab.vue')).toContain('Local-only memory on this device; sign in for cross-device memory')
    expect(src('src/components/settings/tabs/AISettingsTab.vue')).toContain('Missing: {{ aiMemoryDebug.schemaMissingTables.join')
