...
describe('AI sidebar-first desktop experience', () => {
  ...
  it('documents weekly planning as a structured artifact instead of prose plus detached cards', () => {
    ...
    expect(aiChat).toContain('const immediatePlan = buildQuickDraftWeeklyPlan(weekContext)')
    expect(aiChat).toContain('weeklyPlan: immediatePlan')
    expect(aiChat).toContain('weeklyPlan: finalPlan')
    expect(aiChat).toContain('const finalPlan = weeklyPlan ?? immediatePlan')
