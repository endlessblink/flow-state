...
import {
  useBackupSystem,
  BackupData,
} from "../../src/composables/useBackupSystem";
import {
...
describe("TASK-332: Backup Reliability & Verification", () => {
  ...
  beforeEach(() => {
    ...
    mockSafeCreateTask.mockResolvedValue({
      status: "created",
      message: "Created",
    });
  ...
  describe("Data Completeness", () => {
    it("binds an authenticated backup to its source account", async () => {
      ...
      expect(
        (backup as (BackupData & { source?: unknown }) | null)?.source,
      ).toEqual({
        kind: "account",
        userId: "user-1",
      });
    ...
    it("captures projects and groups from every accessible remote scope", async () => {
      ...
      mockProjectStore.projects = [
        { id: "stale-project", name: "Stale current view" },
      ];
      mockCanvasStore.groups = [
        { id: "stale-group", name: "Stale current view" },
      ];
    ...
    it("keeps personal recovery available when an absolute backup also contains shared data", async () => {
      ...
      expect(analysis.canProceed).toBe(true);
      expect(analysis.tasks.toRestore.map((task) => task.id)).toEqual([
        "task-1",
      ]);
      expect(analysis.tasks.toRestore[0]).toEqual(
        expect.objectContaining({
          projectId: "",
          parentTaskId: null,
          parentId: undefined,
      ...
      expect(analysis.tasks.skipped).toEqual(
      ...
      expect(analysis.warnings.join(" ")).toContain(
        "1 shared workspace project",
      );
      expect(analysis.warnings.join(" ")).toContain("1 shared workspace group");
      expect(analysis.warnings.join(" ")).not.toContain(
        "projects will be skipped (deleted or tombstoned)",
      );
      expect(analysis.warnings.join(" ")).not.toContain(
        "groups will be skipped (deleted or tombstoned)",
      );
    ...
    it("restores only the personal partition of a checksummed mixed-scope artifact", async () => {
      ...
      expect(backupSystem.state.value.warning).toContain(
    ...
    it("does not let explicit workspace tombstones block personal project or group recovery", async () => {
      const backup = createMockBackup(0, Date.now(), {
        projects: [
          { id: "project-1", name: "Personal project", workspaceId: null },
        ],
      ...
      expect(analysis.warnings).not.toContain(
        "1 projects will be skipped (deleted or tombstoned)",
      );
      expect(analysis.warnings).not.toContain(
        "1 groups will be skipped (deleted or tombstoned)",
      );
    });
    ...
    it("refuses to claim an emergency backup exists when durable storage rejects it", async () => {
      ...
      expect(backupSystem.state.value.error).toContain(
    ...
    it("keeps the newest recovery point when quota pressure trims backup history", async () => {
      mockTaskStore.tasks = [
        { id: "quota-task", title: "Quota task", status: "todo" },
      ];
    ...
    it("includes active, deleted, completion, and workspace task identities in inventory metadata", async () => {
      ...
      expect(backup?.tasks.find((task) => task.id === "active-1")?.title).toBe(
        "Active",
      );
    });
    ...
    it("refuses to publish an incomplete backup when deleted-task inventory cannot be read", async () => {
      ...
      mockFetchTrash.mockImplementation(
        async (options?: { onError?: () => void }) => {
          options?.onError?.();
          return [];
        },
      );
    ...
    it("refuses an artifact that contains both a task and its permanent-delete tombstone", async () => {
      ...
      expect(backup).toBeNull();
      expect(backupSystem.state.value.error).toContain(
        "contradictory permanent-delete inventory",
      );
    ...
    it.each([
      {
        ...
        arrange: () => {
          mockProjectStore.projects = [
            { id: "stale-project", name: "Stale project" },
          ];
        },
      },
      {
      ...
      "refuses a live $entityType and matching permanent-delete tombstone",
      async ({ entityType, entityId, arrange }) => {
        arrange();
    ...
    it("refuses to publish a backup when permanent-delete inventory cannot be read", async () => {
      mockFetchTombstones.mockImplementation(
        async (options?: { onError?: () => void }) => {
          options?.onError?.();
          return [];
        },
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle missing metadata in backup", async () => {
      ...
      localStorageMock.setItem(
        ...
        JSON.stringify(backupWithoutMetadata),
      );
  ...
  describe("Golden Backup Age Validation", () => {
    it("should warn if golden backup is older than 7 days", async () => {
      ...
      expect(validation?.warnings.some((w) => w.includes("days old"))).toBe(
        true,
      );
    });
  ...
  describe("Golden Backup Rotation (TASK-332)", () => {
    it("should keep up to 3 golden backups in rotation", () => {
      ...
      localStorageMock.setItem(
        "flowstate-backup-golden-rotation",
      ...
      localStorageMock.setItem(
        "flowstate-backup-golden-rotation",
        JSON.stringify([backup2, backup1]),
      );
      useBackupSystem(); // Trigger potential migration

      localStorageMock.setItem(
        "flowstate-backup-golden-rotation",
        JSON.stringify([backup3, backup2, backup1]),
      );
      useBackupSystem(); // Trigger potential migration
      ...
      localStorageMock.setItem(
        "flowstate-backup-golden-rotation",
        JSON.stringify([backup4, backup3, backup2]),
      );
    ...
    it("should return golden backups sorted by task count descending", () => {
      ...
      localStorageMock.setItem(
        "flowstate-backup-golden-rotation",
        JSON.stringify([backup1, backup2, backup3]),
      );
    ...
    it("should migrate legacy single golden backup to rotation", () => {
      ...
      const rotationStored = localStorageMock.getItem(
        "flowstate-backup-golden-rotation",
      );
  ...
  describe("Restore Analysis (Dry-Run)", () => {
    ...
    it("should identify tasks that already exist", async () => {
      ...
      mockCheckTaskIdsAvailability.mockResolvedValue([
        ...
        },
        {
          taskId: "task-4",
          status: "tombstoned",
          reason: "Task permanently deleted",
        },
      ...
      expect(analysis.warnings).toContain(
        "1 tasks already exist (active) - will be skipped",
      );
    });
    ...
    it("fails closed for tombstones without explicit personal provenance", async () => {
      ...
      backup.tombstones = [
        ...
        {
          entityType: "task",
          entityId: "missing-null-personal-task",
          scopeKind: "personal",
        },
        {
          entityType: "task",
  ...
  describe("Restore execution fails closed", () => {
    ...
    it.each([
      ...
      "rejects a $name before any restore mutation",
      async ({ tasks, error }) => {
        const incoming = createMockBackup(0, Date.now(), {
          tasks: tasks as any[],
          metadata: {
    ...
    it("rejects a child whose artifact parent is tombstoned before restoring unrelated tasks", async () => {
      ...
      expect(restored).toBe(false);
      expect(backupSystem.state.value.error).toContain(
        "omitted parent tombstoned-parent",
      );
      expect(mockSafeCreateTask).not.toHaveBeenCalled();
      expect(mockSaveProjects).not.toHaveBeenCalled();
      expect(mockSaveGroup).not.toHaveBeenCalled();
    ...
    it("rejects a child whose omitted parent is only present in trash", async () => {
      ...
      mockCheckTaskIdsAvailability.mockResolvedValue([
        {
          taskId: "trashed-parent",
          status: "soft_deleted",
          reason: "Parent is in trash",
        },
    ...
    it("accepts exact deleted-state readback when recovering a soft-deleted task", async () => {
      ...
      mockCheckTaskIdsAvailability.mockResolvedValue([
        {
          taskId: "task-1",
          status: "soft_deleted",
          reason: "Deleted task is readable in Trash",
        },
      ]);
      ...
      expect(restored).toBe(true);
      expect(mockSafeCreateTask).toHaveBeenCalledWith(
        expect.objectContaining({
          id: "task-1",
          _soft_deleted: true,
        }),
    ...
    it("restores only explicit personal tombstones during the non-atomic fallback path", async () => {
      ...
      mockFetchTombstones
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([
          { entityType: "task", entityId: "personal-gone-task" },
        ]);
    ...
    it("refuses a current-schema artifact with no source-account provenance", async () => {
      const backup = createMockBackup(1) as BackupData & {
        source?: BackupData["source"];
      };
      delete backup.source;

      const restored = await backupSystem.restoreBackup(backup as BackupData, {
        skipDedupCheck: true,
      });
  ...
  describe("Recovery entrypoints preserve validated artifacts", () => {
    ...
    it("adds a valid checksum before restoring a shadow snapshot", async () => {
        ...
        .mockResolvedValueOnce([
          {
            taskId: "shadow-task",
            status: "active",
            reason: "Task is readable",
          },
        ]);
      const shadow = {
        source: { kind: "account", userId: "user-1" },
        tasks: [
          { id: "shadow-task", title: "Shadow recovery", status: "todo" },
        ],
        projects: [],
        groups: [],
        meta: {
      ...
      expect(restored).toBe(true);
      expect(mockSafeCreateTask).toHaveBeenCalledWith(
        expect.objectContaining({ id: "shadow-task" }),
      );
    });
