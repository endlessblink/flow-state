...
const mkGroup = (id: string, name: string, x: number): CanvasGroup =>
  ({
    ...
    name,
    isVisible: true,
    position: { x, y: 0, width: 350, height: 600 },
...
describe("layout no-op skip (TASK-1871)", () => {
  ...
  it("updateGroup skips the write when position is unchanged (no version bump)", async () => {
    ...
    await canvasStore.updateGroup("grp-x", {
      position: { x: 100, y: 0, width: 350, height: 600 },
    });
    expect(
      canvasStore.groups.find((x) => x.id === "grp-x")?.positionVersion,
    ).toBe(5);

    // Real change → version advances.
    await canvasStore.updateGroup("grp-x", {
      position: { x: 500, y: 0, width: 350, height: 600 },
    });
    expect(
      canvasStore.groups.find((x) => x.id === "grp-x")?.positionVersion,
    ).toBe(6);
