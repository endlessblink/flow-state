...
describe('electron-builder dependency parser patch', () => {
  it('patches the polluted JSON parser start index instead of forcing traversal mode', () => {
    ...
    expect(patchScript).toContain('dependencyTreeInsertionPointAfterArgs')
    expect(patchScript).toContain("applied.push('precomputed-tree')")
  ...
  it('runs the patch before every tracked Electron packaging path', () => {
    ...
    expect(builderWrapper).toContain('"$ROOT_DIR/node_modules/.bin/electron-builder" --config "$ROOT_DIR/electron-builder.yml"')
    expect(builderWrapper).toContain("const start=raw.indexOf('{')")
    expect(builderWrapper).toContain("const end=raw.lastIndexOf('}')")
    expect(builderWrapper).toContain("npm list produced no JSON dependency tree")
