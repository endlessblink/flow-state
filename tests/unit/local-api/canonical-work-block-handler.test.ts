...
describe('canonical Local API work-block handler', () => {
  ...
  it.each([
    ...
    ['resize', 3, { action: 'resize', workBlockId, duration: 90, finishBy: '2026-07-17T11:00' }],
    ['remove', 3, { action: 'remove', workBlockId }],
  ...
  it('rejects malformed action-specific commands before the RPC', async () => {
    ...
    const invalid = [
      ...
      { revision: 1, command: { action: 'resize', workBlockId, duration: 90, timezone: 'UTC' } },
