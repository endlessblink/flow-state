...
async function startFakePostgrest() {
  ...
  const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.method === 'POST' && url.pathname === '/rest/v1/rpc/flowstate_subtask_batch_v1') {
      ...
      const existing = Array.isArray(row.subtasks) ? row.subtasks as TaskRow[] : []
...
async function startSignedSidecar(entry: string, supabaseUrl: string) {
  const port = await unusedPort()
  const dataDir = mkdtempSync(join(tmpdir(), 'flowstate-exact-task-signed-runtime-'))
...
async function getSubtasks(port: number, query = '') {
  const response = await fetch(`http://127.0.0.1:${port}/api/tasks/${TASK_ID}/subtasks${query}`, {
    headers: {
      Host: `127.0.0.1:${port}`,
      Authorization: `Bearer ${TOKEN}`,
    },
  ...
  for (const [label, artifact] of artifacts) {
    it(`${label} returns only fresh ordered canonical subtasks with parent revision authority`, async () => {
      ...
      try {
        fake.setRow(baseTask({
          ...
          subtasks: [
            { id: 'step-a', clientId: 'draft-a', title: 'First step', order: 0 },
            { id: 'step-b', clientId: 'draft-b', title: 'Second step', order: 1 },
          ],
        }))
        ...
        expect(refreshed.body.page).toEqual({ limit: 100, total: 1, hasMore: false, nextCursor: null })
    ...
    it(`${label} pages validated canonical subtasks and rejects a cursor after the parent revision changes`, async () => {
      ...
      try {
        ...
        expect(first.body.page).toMatchObject({ limit: 100, total: 101, hasMore: true })
        const nextCursor = (first.body.page as TaskRow).nextCursor
        expect(typeof nextCursor).toBe('string')

        const second = await getSubtasks(sidecar.port, `?cursor=${encodeURIComponent(String(nextCursor))}`)
        ...
        const stale = await getSubtasks(sidecar.port, `?cursor=${encodeURIComponent(String(nextCursor))}`)
    ...
    it(`${label} preserves legacy singular preview requests without allowing receipt-free apply`, async () => {
      ...
      try {
        ...
        const preview = await createSubtask(sidecar.port, { title: 'Legacy preview step' })
    ...
    it(`${label} fails closed when stored subtask identities or ordering are malformed`, async () => {
      ...
      try {
        ...
        const invalidSubtasks = [
          ...
          [
            { id: 'step-a', clientId: 'duplicate-client', title: 'First', order: 0 },
            { id: 'step-b', clientId: 'duplicate-client', title: 'Second', order: 1 },
          ],
          [{ id: 'step-a', title: 'Wrong order', order: 2 }],
          [{ id: 'step-a', title: 'Unknown field', order: 0, raw: true }],
          Array.from({ length: 101 }, (_, order) => order === 100
