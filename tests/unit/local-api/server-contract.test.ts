...
describe('Local API sidecar timer endpoint regression contract', () => {
  ...
  it('reads fresh mutation-ready task instances without returning full task bodies', () => {
    const body = functionBody('handleGetTaskInstances')

    expect(body).toContain('readWorkBlockInventory(ctx, id)')
    expect(body).toContain('send(res, result.status, result.body)')
    expect(body).not.toContain(".from('tasks')")
    expect(body).not.toContain(".eq('user_id', userId)")
    expect(body).not.toContain('accessToken')
    expect(body).not.toContain('refreshToken')
    expect(body).not.toContain('anonKey')
  ...
  it('routes singular task-instance creation through the canonical batch without direct writes', () => {
    const body = functionBody('handlePostTaskInstance')

    expect(body).toContain("kind: 'create'")
    expect(body).toContain('taskId: id')
    expect(body).toContain('baseRevision: body.baseRevision')
    expect(body).toContain('clientId: body.clientId || body.instanceId')
    expect(body).toContain('body.operationId = body.operationId || body.requestId')
    expect(body).toContain('handleCanonicalWorkBlockBatch')
    expect(body).not.toContain(".from('tasks')")
    expect(body).not.toContain('.update(')
  })

  it('routes move, resize, and remove through the same canonical adapter', () => {
    const patch = functionBody('handlePatchTaskInstance')
    const deletion = functionBody('handleDeleteTaskInstance')
    const canonical = functionBody('handleCanonicalWorkBlockBatch')

    expect(patch).toContain("body.scheduledDate !== undefined || body.scheduledTime !== undefined ? 'move' : 'resize'")
    expect(patch).toContain('baseWorkBlockHash: body.baseWorkBlockHash')
    expect(patch).toContain("...(kind === 'move' && body.duration !== undefined && { duration: body.duration })")
    expect(deletion).toContain("kind: 'remove'")
    expect(deletion).toContain('baseWorkBlockHash: body.baseWorkBlockHash')
    expect(canonical).toContain('executeWorkBlockBatch')
    expect(canonical).toContain('activeWorkspaceId: ctx.activeWorkspaceId')
    expect(canonical).toContain('signedUser: ctx.signedUser')
    for (const source of [patch, deletion, canonical]) {
      expect(source).not.toContain(".from('tasks')")
      expect(source).not.toContain('.update(')
    }
  })

  it('keeps canonical work-block responses free of locally reconstructed task data', () => {
    const body = functionBody('handleCanonicalWorkBlockBatch')
  ...
  it('routes singular subtask mutations through the canonical preview/apply batch', () => {
    ...
    expect(create).toContain('clientId: body.clientId')
