...
function preview(overrides: Record<string, unknown> = {}) {
  return {
    ...
    normalizedPayload: {
      taskId,
      operations: JSON.parse(JSON.stringify(requestedOperations)),
    },
    readBack: {
      id: taskId,
      workspaceId,
...
describe('TASK-1963 canonical Local API subtask batch', () => {
  ...
  describe.skipIf(!moduleExists)('executeSubtaskBatch contract', () => {
    ...
    it('rejects enriched create operations but accepts deep-equal canvas positions', async () => {
      const enriched = requestedOperations.map((operation) => operation.kind === 'create'
        ? { ...operation, subtaskId: 'generated-step', parentTaskId: taskId, isCompleted: false }
        : JSON.parse(JSON.stringify(operation)))
      const forged = preview({ normalizedPayload: { taskId, operations: enriched } })
      const invalid = harness(forged)

      const rejected = await invalid.executeSubtaskBatch(
        context(invalid.rpc), taskId,
        { operationId, baseRevision: 7, operations: requestedOperations },
        invalid.notifyTaskMutation,
      )
      expect(rejected.status).toBe(502)

      const exact = preview()
      const valid = harness(exact)
      const accepted = await valid.executeSubtaskBatch(
        context(valid.rpc), taskId,
        { operationId, baseRevision: 7, operations: requestedOperations },
        valid.notifyTaskMutation,
      )
      expect(accepted).toEqual({ status: 200, body: exact })
    })
