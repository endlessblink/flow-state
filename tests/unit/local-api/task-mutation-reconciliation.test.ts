...
describe('BUG-1938 Local Task API renderer reconciliation', () => {
  ...
  it('reloads both recurring rows after a successful Done for now apply', () => {
    const doneForNow = functionSlice(server, 'handleDoneForNow', 'handleGetTaskInstances')

    expect(doneForNow).toContain("notifyTaskMutation('update', id)")
    expect(doneForNow).toContain('if (!preview)')
    expect(doneForNow).toContain('const { state: _rendererState, ...safeResult } = data')
    expect(appInitialization).toContain('invalidateCache.tasks()')
    expect(appInitialization).toContain('taskStore.loadFromDatabase()')
  })

  it('reloads the survivor and removes the archived duplicate after merge apply', () => {
    const merge = functionSlice(server, 'handleMergeTasks', 'handleGetTaskInstances')

    expect(merge).toContain("notifyTaskMutation('update', survivorTaskId)")
    expect(merge).toContain("notifyTaskMutation('delete', duplicateTaskId)")
    expect(merge).toContain('if (!preview)')
    expect(appInitialization).toContain('invalidateCache.tasks()')
    expect(appInitialization).toContain('taskStore.loadFromDatabase()')
  })
