...
describe('TASK-1949 canonical assistant disposable reliability harness', () => {
  ...
  it('applies the full canonical migration chain before executable task, Notion, and race proofs', () => {
    ...
    expect(source).toContain('20260716010000_canonical_recurrence_lifecycle.sql')
    expect(source).toContain('20260716020000_canonical_timer_command.sql')
    expect(source).toContain('20260716030000_canonical_organization_commands.sql')
    ...
    expect(source).toContain('test-recurrence-lifecycle-rpc.sql')
    expect(source).toContain('test-canonical-timer-command.sql')
    expect(source).toContain('test-canonical-organization-commands.sql')
