...
describe('production deployment convergence gates', () => {
  ...
  it('serializes the full ship gate and restores Doppler build input afterward', () => {
    ...
    expect(workflow).toContain('Build and deploy Electron update')
    expect(workflow).toContain('deploy script stages the PWA, Electron artifacts')
    expect(workflow).not.toContain('Deploy Static Files')
    expect(workflow).not.toContain('Rebuild PWA release surface after Electron packaging')
  })
})
