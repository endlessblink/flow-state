...
function makeReleaseFixture(version = '1.4.262') {
  ...
  write(root, 'release/latest-linux.yml', [
    `version: ${version}`,
    'files:',
...
describe('FlowState source-to-runtime truth ledger', () => {
  ...
  it('builds a redacted non-live ledger without probing public, installed, or sidecar surfaces', async () => {
    ...
    const probePublic = vi.fn(() => { throw new Error('must not run') })
    const probeInstalled = vi.fn(() => { throw new Error('must not run') })
    const probeSidecar = vi.fn(() => { throw new Error('must not run') })
    ...
    expect(ledger.build.manifest.artifacts[0]).toEqual(expect.objectContaining({
  ...
  it('reports version and sidecar digest mismatches independently in full mode', async () => {
    ...
    const ledger = await buildTruthLedger({
      ...
      probePublic: async () => ({ status: 'available', version: '1.4.261', artifactsReachable: true }),
      probeInstalled: async () => ({ status: 'available', version: '1.4.260', sha256: 'c'.repeat(64) }),
      probeSidecar: async () => ({ status: 'available', health: true, appVersion: '1.4.259', bundleSha256: 'd'.repeat(64) }),
    })
    ...
    expect(ledger.verdict.mismatches).toEqual([
      ...
      { surface: 'public', field: 'version', expected: '1.4.262', actual: '1.4.261' },
      { surface: 'installed', field: 'version', expected: '1.4.262', actual: '1.4.260' },
      { surface: 'sidecar', field: 'appVersion', expected: '1.4.262', actual: '1.4.259' },
      { surface: 'sidecar', field: 'bundleSha256', expected: ledger.build.sidecar.sha256, actual: 'd'.repeat(64) },
    ])
  })
  ...
  it('keeps live probe failures typed instead of treating them as mismatches', async () => {
    ...
    const ledger = await buildTruthLedger({
      ...
      probePublic: async () => ({ status: 'unavailable', error: 'request_failed' }),
      probeInstalled: async () => ({ status: 'unavailable', error: 'not_found' }),
      probeSidecar: async () => ({ status: 'unavailable', error: 'connection_refused' }),
    })

    expect(ledger.verdict).toEqual({ consistent: null, mismatches: [] })
    expect(ledger.public).toEqual({ status: 'unavailable', error: 'request_failed' })
  })

  it('uses the dedicated redacted sidecar provenance contract instead of timer diagnostics', () => {
    const root = resolve(__dirname, '../../..')
    const source = readFileSync(join(root, 'scripts/flowstate-truth-ledger.cjs'), 'utf8')
  ...
  it('is generated as a non-live release output by the canonical Electron builder', () => {
    const root = resolve(__dirname, '../../..')
    const buildScript = readFileSync(join(root, 'scripts/run-electron-builder-with-npm-tree.sh'), 'utf8')
