...
describe('VPS Electron release branch contract', () => {
  it('validates the Doppler credential before the locked package build', () => {
    expect(releaseScript).toContain("/usr/bin/doppler run -- env npm_config_production=false '${NODE_BIN}/npm' run electron:validate-env;")
    expect(releaseScript.indexOf('run electron:validate-env')).toBeLessThan(
      releaseScript.indexOf('run electron:build:locked')
    )
  })
