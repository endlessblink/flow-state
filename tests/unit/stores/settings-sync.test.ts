...
describe('Settings Supabase sync', () => {
  ...
  it('strips googleProviderRefreshToken from the settings blob', async () => {
    ...
    const [upsertPayload] = mockUpsert.mock.calls[0]
    expect(upsertPayload.settings).not.toHaveProperty('googleProviderRefreshToken')
  })
  ...
  it('strips googleProviderTokenExpiry from the settings blob', async () => {
    ...
    const [upsertPayload] = mockUpsert.mock.calls[0]
    expect(upsertPayload.settings).not.toHaveProperty('googleProviderTokenExpiry')
  })
  ...
  it('includes googleCalendars in the settings blob', async () => {
    ...
    store.$patch({
      googleCalendars: [
        { id: 'cal-1', summary: 'Work', backgroundColor: '#0f0', enabled: true },
      ],
    })
  ...
  it('includes showGoogleCalendarEvents in the settings blob', async () => {
    ...
    const [upsertPayload] = mockUpsert.mock.calls[0]
    expect(upsertPayload.settings).toHaveProperty('showGoogleCalendarEvents', false)
  })
  ...
  it('logs success message when upsert succeeds', async () => {
    ...
    const successCalls = logSpy.mock.calls.filter(([msg]) =>
  ...
  it('logs error when upsert returns an error object', async () => {
    ...
    const errorCalls = errorSpy.mock.calls.filter(([msg]) =>
      typeof msg === 'string' && msg.includes('[SETTINGS] Supabase upsert failed')
    )
  ...
  it('schedules a retry via setTimeout when getUser returns no user', async () => {
    ...
    const retryCalls = warnSpy.mock.calls.filter(([msg]) =>
      typeof msg === 'string' &&
      (msg.includes('No authenticated user') || msg.includes('Retry: still no authenticated user'))
    )
