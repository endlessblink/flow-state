...
vi.mock('@/stores/tasks', () => ({
  useTaskStore: () => ({ loadFromDatabase: vi.fn().mockResolvedValue(undefined) }),
}))
...
function buildSelectChain(result: { data: unknown; error: unknown }) {
  ...
  Object.defineProperty(chain, Symbol.for('__PROMISE__'), { get: () => Promise.resolve(result) })
...
describe('useWorkspaceStore', () => {
  ...
  it('4. loadWorkspaces handles missing table error (PGRST205) gracefully', async () => {
    const pgrst205Error = { code: 'PGRST205', message: 'Relation not found', details: null, hint: null }
  ...
  it('8. loadMembers populates the members Map for a given workspace', async () => {
    const memberRows = [
      { id: 'm1', workspace_id: 'ws-1', user_id: 'user-456', role: 'owner', joined_at: new Date().toISOString() },
    ]
    const memberChain = {
      select: vi.fn().mockReturnThis(),
  ...
  it('9. generateInviteLink creates an invite record and returns a link', async () => {
    ...
    const link = await store.generateInviteLink('ws-abc', 'colleague@example.com', 'member')
