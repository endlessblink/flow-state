...
const CREATE_ONLY = new Set<string>([
  'id', 'user_id', 'created_at', 'updated_at', // identity/timestamps
  'deleted_at', 'is_deleted',                  // delete path, not update
  'is_uncategorized',                          // derived from project_id in update path
  'format', 'x', 'y',                          // legacy/position-json artifacts (position handles geometry on update)
...
describe('task sync payload completeness (TASK-1871)', () => {
  it('every create-mapper column is covered by the update path (or explicitly create-only)', () => {
    ...
    expect(createCols.size, 'failed to parse create mapper columns').toBeGreaterThan(10)

    const uncovered = [...createCols].filter((c) => !updateCols.has(c) && !CREATE_ONLY.has(c))
    expect(
      uncovered,
