...
describe("undo-aware modal and context-menu entry points", () => {
  it("keeps TaskContextMenu user mutations on undo-aware APIs", () => {
    ...
    expect(source).toContain(
      "await taskStore.updateTaskWithUndo(currentTask.value.id, { isPinned:",
    );
    expect(source).toContain(
      "await taskStore.updateTaskWithUndo(currentTask.value.id, { calendarLocked:",
    );
    ...
    expect(source).not.toContain(
      "await taskStore.updateTask(currentTask.value.id, { isPinned:",
    );
    expect(source).not.toContain(
      "await taskStore.updateTask(currentTask.value.id, { calendarLocked:",
    );
  ...
  it("routes canvas-origin permanent task delete through a REAL hard delete (BUG-1850)", () => {
    ...
    expect(canvasTaskActions).toContain(
      "detail: { taskId: task.id, permanent: false, context: 'canvas' }",
    );
    expect(canvasHotkeys).toContain(
    ...
    expect(canvasTaskActions).not.toContain(
    ...
    expect(modalManager).not.toContain(
  ...
  it("keeps direct task edit and quick-create entry points undo-aware", () => {
    ...
    expect(editActions).not.toContain(
  ...
  it("wires every TaskEditModal permanent-delete event to a real handler", () => {
    ...
    for (const file of files) {
      ...
      expect(source, `${file} should render TaskEditModal`).toContain(
        "<TaskEditModal",
      );
      expect(
        source,
        `${file} must handle TaskEditModal permanent-delete`,
      ).toContain("@permanent-delete=");
    }
  });
  ...
  it("keeps small inline task edits undo-aware", () => {
    ...
    const taskCardActions = readSource(
      "src/composables/tasks/card/useTaskCardActions.ts",
    );
    const taskAssignment = readSource(
      "src/composables/workspace/useTaskAssignment.ts",
    );

    expect(quickSort).toContain(
      "await taskStore.updateTaskWithUndo(currentTask.value.id, updates)",
    );
    expect(quickSort).not.toContain(
    ...
    expect(taskAssignment).not.toContain(
  ...
  it("keeps quick task creation and mini-canvas edits undo-aware", () => {
    ...
    const morningQuickCapture = readSource(
      "src/components/morning-dashboard/MorningQuickCapture.vue",
    );
    const bigThree = readSource(
      "src/components/morning-dashboard/BigThreeCard.vue",
    );
    const miniCanvas = readSource(
      "src/composables/mini-canvas/useMiniCanvasActions.ts",
    );

    expect(quickTasks).toContain("await taskStore.createTaskWithUndo({");
    expect(quickTasks).toContain(
  ...
  it("keeps calendar scheduling, resizing, and date moves undo-aware", () => {
    ...
    const contextMenuActions = readSource(
      "src/composables/tasks/useTaskContextMenuActions.ts",
    );
    ...
    expect(dayView).not.toContain(
      "await taskStore.updateTask(taskId, { isInInbox: false })",
    );
    expect(dayView).not.toContain("taskStore.createTask({");
    expect(dayView).not.toContain(
      "taskStore.updateTaskInstance(calendarEvent.taskId",
    );
    ...
    expect(vueCalView).toContain(
      "await taskStore.updateTaskWithUndo(event.id, {",
    );
    expect(vueCalView).not.toContain("await taskStore.updateTask(event.id, {");

    expect(calendarCore).toContain(
      "await taskStore.updateTaskWithUndo(task.id, {",
    );
    expect(calendarCore).not.toContain(
      "await taskStore.moveTask(task.id, nextStatus)",
    );
    expect(calendarCore).not.toContain(
      "taskStore.updateTaskInstance(calendarEvent.taskId",
    );

    expect(contextMenuActions).toContain(
      "await updateDueDateWithCalendarInstance(taskId,",
    );
    expect(contextMenuActions).not.toContain(
      "await taskStore.updateTaskInstance(taskId, calendarInstanceId",
    );
    expect(contextMenuActions).toContain(
