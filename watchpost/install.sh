#!/bin/bash
# Watchpost Install/Update Script
# Usage: curl -sSL https://raw.githubusercontent.com/endlessblink/watchpost/main/install.sh | bash
...
EOF
    ...
    # 2. Create watchpost.sh launcher script
    cat > "$PROJECT_ROOT/watchpost.sh" << 'LAUNCHER'
#!/bin/bash
# Watchpost Launcher
# Starts Watchpost with this project's MASTER_PLAN.md

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INSTALL_DIR="$HOME/.watchpost"

if [ ! -d "$INSTALL_DIR" ]; then
    echo "Watchpost not installed. Installing..."
    curl -sSL https://raw.githubusercontent.com/endlessblink/watchpost/main/install.sh | bash
fi

# Export project root so Watchpost knows where to find MASTER_PLAN.md
...
CLAUDEMD
            ...
            echo -e "${YELLOW}Watchpost section already in CLAUDE.md${NC}"
        fi
    fi
fi
...
echo -e "║    alias watchpost='cd $INSTALL_DIR && npm start'          ║"
echo -e "║                                                            ║"
